\chapter{複素数}
    \section{複素数の定義}
        ...
        \begin{align*}
            ...
            \textrm{実数倍}\;(a\in\realNumbers) &: a(a_1,b_1) = (aa_1,ab_1)\\
        ...
        なぜならば，
        \begin{align*}
            (a_1+ib_1)(a_2+ib_2) &= \bracks*{(a_1,0)+i(b_1,0)}\times\bracks*{(a_2,0)+i(b_2,0)} \\
            &= \bracks*{(a_1,0)+(0,1)\times(b_1,0)}\times\bracks*{(a_2,0)+(0,1)\times(b_2,0)} \\
            &= \bracks*{(a_1,0)+(0,b_1)}\times\bracks*{(a_2,0)+(0,b_2)} \\
        ...
        \subsection{コラム\texorpdfstring{$i^2=-1\;?$}{虚数単位の2乗は-1?}}
            ...
            百歩譲って実数の集合$\realNumbers$上で定義された「2乗」と解釈したとしても，実数の集合に属さない数に対してこの演算を適用できる保証など無い。
            ...
            しかし数学的に「ちゃんとした」理論を作るためには，既存の枠組みのままでは不可能だと分かり次第，思い切ってルールを拡張し，所望の性質が得られるように理屈を先回りして演算を機械的に定義するようなbreak throughが必要なこともある。
    ...
    \section{諸公式}
        \subsection{\texorpdfstring{$x>0,z\in\complexNumbers$}{x>0, zは複素数}のとき\texorpdfstring{$|x^z|=x^\Re{z}$}{x\string^zの絶対値はx\string^Re(z)}}
            ...
            \end{proof}
        \subsection{\texorpdfstring{$^\forall n\nmid k,\sum_{l=1}^n{\exp\parens*{i\frac{k}{n}2\pi l}} = 0$}{kはnの倍数でないとする。exp(ikl2π/n)をl=1,2...,nについて足すと0である。}}
            \label{fml:花びら定理}
            \begin{proof}
                \begin{align*}
                    \sum_{l=1}^n{\exp\parens*{i\frac{k}{n}2\pi l}} &= \frac{\exp\parens*{i\frac{k}{n}2\pi}\parens*{1-\exp\parens*{i\frac{k}{n}2\pi}^n}}{1-\exp\parens*{i\frac{k}{n}2\pi}} = \frac{\exp\parens*{i\frac{k}{n}2\pi}\parens*{1-\NapierE^{ik2\pi}}}{1-\exp\parens*{i\frac{k}{n}2\pi}} =0
                \end{align*}
            \end{proof}
    \section{代数方程式}
        \subsection{諸定理}
