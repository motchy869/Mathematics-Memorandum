\chapter{Fourier級数}
    \section{定義と背景の整理}
        $T>0$周期関数$f:\realNumbers\to\realNumbers$に対して，次式を$f$のFourier級数と呼ぶ。
        ...
        \subsection{無限次元ベクトル空間との関係}
            区間$[-T/2,T/2]$上のRiemann可積分な実数値関数全体の成すベクトル空間$V$上の内積$\inProd{\cdot}{\cdot}:V\times V\to\realNumbers$を次式で定義する。
            ...
            前述のFourier級数の定義では$\cos,\sin$の項は基底をFourier係数で重み付けたものであるが，定数項だけは基底の$1/\sqrt{2}$倍に係数が掛かっており首尾一貫しない。
    ...
    \section{連続関数のFourier級数の高周波成分は0に収束する}
        ...
        \begin{proof}
            ...
            \begin{align*}
                \integrate{0}{\pi}{f(x)\sin kx}{}{x} &= \sum_{l=0}^{\floor{k/2}-1}\parens*{\integrate{2l\pi/k}{(2l+1)\pi/k}{f(x) \sin kx}{}{x} + \integrate{(2l+1)\pi/k}{(2l+2)\pi/k}{f(x) \sin kx}{}{x}} \\
                &\quad + \integrate{2\floor{k/2}\pi/k}{\pi}{f(x)\sin kx}{}{x}
            \end{align*}
            最後の項は$k\to\infty$のとき$0$に収束する。
            最初の2項は$y=kx$と変数変換して
            \begin{align*}
                &\quad \sum_{l=0}^{\floor{k/2}-1}\frac{1}{k}\parens*{\integrate{2l\pi}{(2l+1)\pi}{f(y/k) \sin y}{}{y} + \integrate{(2l+1)\pi}{(2l+2)\pi}{f(y/k) \sin y}{}{y}} \\
    ...
    \section{Fourier級数の応用}
        ...
        \begin{proof}
            ...
            \begin{align*}
                ...
                &= \lim_{n\to\infty}\frac{1}{2}\integrate{-\pi}{\pi}{\frac{\sin nt}{t}}{}{t} = \lim_{n\to\infty}\frac{1}{2}\integrate{-\pi}{\pi}{\parens*{\frac{1}{t} - \frac{1}{2\tan(t/2)} + \frac{1}{2\tan(t/2)}}\sin nt}{}{t}
            \end{align*}
            $\frac{1}{t} - \frac{1}{2\tan(t/2)}$は$[-\pi,\pi]$で連続だから\ref{連続関数のFourier 級数の高周波成分は0に収束する}より
            \[ \lim_{n\to\infty}\frac{1}{2}\integrate{-\pi}{\pi}{\parens*{\frac{1}{t} - \frac{1}{2\tan(t/2)}}\sin nt}{}{t} = 0 \]
