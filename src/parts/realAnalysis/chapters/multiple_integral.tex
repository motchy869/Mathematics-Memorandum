\chapter{重積分}
    \section{平均値の定理}
        \begin{shadebox}
            $\Omega\in\realNumbers^n$を体積確定の有界閉領域，$f:\Omega\to\realNumbers$を連続関数とすると，$^\exists\bm{x}'\st\integrate{\Omega}{}{f(\bm{x})}{n}{x} = f(\bm{x}')|\Omega|$
        \end{shadebox}
        \begin{proof}
            \quad\par
            $\Omega$内での$f$の最小値と最大値を各々$m(\Omega),M(\Omega)$とすると
            \[m(\Omega)|\Omega| \leq \integrate{\Omega}{}{f(\bm{x})}{n}{x} \leq M(\Omega)|\Omega|\quad\therefore\;m(\Omega) \leq \frac{1}{|\Omega|}\integrate{\Omega}{}{f(\bm{x})}{n}{x} \leq M(\Omega)\]
    ...
    \section{重み付き Cauchy-Schwarz の不等式}
        \begin{shadebox}
            ...
            このとき，次式が成り立つ。
            \[ \parens*{\integrate{D}{}{w(\bm{x})f(\bm{x})g(\bm{x})}{n}{x}}^2 \leq \parens*{\integrate{D}{}{w(\bm{x})f(\bm{x})^2}{n}{x}} \parens*{\integrate{D}{}{w(\bm{x})g(\bm{x})^2}{n}{x}} \]
        \end{shadebox}
