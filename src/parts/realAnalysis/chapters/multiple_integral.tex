\chapter{重積分}
    \section{平均値の定理}
        \begin{shadebox}
            $\Omega\in\realNumbers^n$を体積確定の有界閉領域，$f:\Omega\to\realNumbers$を連続関数とすると，$^\exists\vx'\st\integrate{\Omega}{}{f(\vx)}{n}{x} = f(\vx')|\Omega|$
        \end{shadebox}
        \begin{proof}
            \quad\par
            $\Omega$内での$f$の最小値と最大値を各々$m(\Omega),M(\Omega)$とすると
            \[m(\Omega)|\Omega| \leq \integrate{\Omega}{}{f(\vx)}{n}{x} \leq M(\Omega)|\Omega|\quad\therefore\;m(\Omega) \leq \frac{1}{|\Omega|}\integrate{\Omega}{}{f(\vx)}{n}{x} \leq M(\Omega)\]
            であるから中辺の値を$I(\Omega)$とすれば中間値の定理より$^\exists\vx'\st I(\Omega) = f(\vx')$であり，定理が従う。
        \end{proof}
    \section{重み付きCauchy-Schwarzの不等式}
        \begin{shadebox}
            $n\in\naturalNumbers, D$は$\realNumbers^n$の閉領域であるとする。
            $f,g:D\to\realNumbers,w:D\to\nonNegRealNumbers$は連続であるとする。
            このとき，次式が成り立つ。
            \[ \parens*{\integrate{D}{}{w(\vx)f(\vx)g(\vx)}{n}{x}}^2 \leq \parens*{\integrate{D}{}{w(\vx)f(\vx)^2}{n}{x}} \parens*{\integrate{D}{}{w(\vx)g(\vx)^2}{n}{x}} \]
        \end{shadebox}
        Riemann積分に於いて，Riemann和に対して\ref{discrete_weighted_Cauchy_Schwarz_inequality}を適用して証明できる。
    ...
    \section{関数\texorpdfstring{$f,g$}{f,g}が\texorpdfstring{$\realNumbers$}{R}上で有界,連続で\texorpdfstring{$\integrate{-\infty}{\infty}{f(x)}{}{x},\integrate{-\infty}{\infty}{g(x)}{}{x}$}{f,gの-∞から+∞に渡る積分}が絶対収束するなら畳み込み\texorpdfstring{$f*g$}{f*g}は\texorpdfstring{$\realNumbers$}{R}上で連続で絶対可積分である。}
        \begin{proof}
            ...
            \begin{align*}
                \abs{(f*g)(x+h) - (f*g)(x)} &= \abs{\integrate{-\infty}{\infty}{\bracks*{f(x-y+h) - f(x-y)}g(y)}{}{y}} \\
                &\leq \integrate{-\infty}{\infty}{\abs{\bracks*{f(x-y+h) - f(x-y)}g(y)}}{}{y}
            \end{align*}
            $f$の有界性と$g$の絶対可積分性より，ある$a$が存在して
            \[ \integrate{-\infty}{a}{\abs{\bracks*{f(x-y+h) - f(x-y)}g(y)}}{}{y},\integrate{a}{\infty}{\abs{\bracks*{f(x-y+h) - f(x-y)}g(y)}}{}{y} < \varepsilon/3 \]
            となる。$f$は$\realNumbers$上で連続だから特に$[x-a-1,x+a+1]$上で一様連続である。
            よって$h$を十分小さくすれば
            \[ |f(x-y+h) - f(x-y)| < \frac{\varepsilon}{3L} \quad \parens*{L \coloneq \integrate{-\infty}{\infty}{|g(y)|}{}{y} } \]
            となり，
            \[ \integrate{-a}{a}{\abs{\bracks*{f(x-y+h) - f(x-y)}g(y)}}{}{y} < \varepsilon/3 \]
            以上より
            ...
            これより
            \begin{align*}
                I(a) &\leq \integrate{-\infty}{\infty}{\abs{g(y)}\integrate{-a}{a}{\abs{f(x-y)}}{}{x}}{}{y} \leq \integrate{-\infty}{\infty}{\abs{g(y)}M_1}{}{y} \quad \parens*{ M_1 \coloneq \integrate{-\infty}{\infty}{\abs{f(x-y)}}{}{x} } \\
                &\leq M_1M_2 \quad \parens*{ M_2 \coloneq \integrate{-\infty}{\infty}{\abs{g(y)}}{}{y} }
            \end{align*}
    ...
    \section{諸公式}
        \subsection{円座標表示された領域の面積\texorpdfstring{$\integrate{0}{2\pi}{r_x(\theta){r_y}'(\theta)}{}{\theta} = -\integrate{0}{2\pi}{r_y(\theta){r_x}'(\theta)}{}{\theta}$}{}}
            ...
            証明は割愛する。
            左辺については，$\theta$の増加に伴って，第1象限から第4象限にかけて順番に，横長の長方形を$y$軸方向に積み重ねてゆく様子を想像すれば納得できる。
            同様に，右辺についても長方形を積み重ねる向きを変えて理解できる。
        \subsection{\texorpdfstring{$n$}{n}次元の単体の体積\texorpdfstring{$\int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \dotsi \int_{x_n=0}^{a-x_1- \dotsb - x_{n-2}} \dx_n \dotsi \dx_2\dx_1 = a^n/n!\;(a>0)$}{}}
            \begin{proof}
                \begin{align*}
                    &\phantom{=} \int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \dotsi \int_{x_n=0}^{a-x_1- \dotsb - x_{n-2}} \dx_n \dotsi \dx_2\dx_1 \\
                    &= \int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \dotsi \int_{x_{n-1}=0}^{a-x_1- \dotsb - x_{n-3}} (a-x_1- \dotsb - x_{n-1}) \dx_{n-1} \dotsi \dx_2\dx_1 \\
                    &= \frac{1}{2}\int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \dotsi \int_{x_{n-2}=0}^{a-x_1-\dotsb-x_{n-4}} (a-x_1- \dotsb - x_{n-2})^2 \dx_{n-2} \dotsi \dx_2\dx_1 \\
                    &= \dotsb \\
                    &= \frac{1}{2\times 3\times\dotsb(n-1)}\int_{x_1=0}^a (a-x_1)^{n-1} \dx_1 \\
                    &= \frac{a^n}{n!}
                \end{align*}
            \end{proof}