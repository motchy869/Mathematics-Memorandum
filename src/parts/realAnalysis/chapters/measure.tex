\chapter{測度}
    ...
    \section{Jordan測度}
        ...
        \begin{itemize}
            ...
            \[ S(f,\Delta) \coloneq \sum_{r\in\Delta} \sup_{\vx\in r}f(\vx)|r|,\quad s(f,\Delta) \coloneq \sum_{r\in\Delta} \inf_{\vx\in r}f(\vx)|r| \]
            と定義する。
    ...
    \section{Lebesgue測度}
        ...
        \subsection{有界閉集合\texorpdfstring{$A\subset\realNumbers^n$}{A⊂R\string^n}に対して\texorpdfstring{$\mLebO{A}=0 \iff \mJordan{A}=0$}{AのLebesgue外測度=0 ⇔ AのJordan外測度}}
            \begin{proof}
                ...
                $\bigcup_{n=1}^\infty \operatorname{Int}(2E_n)$は$A$の開被覆であり，$A$は$\realNumbers^n$の有界閉集合なのでコンパクトであるから，ある$N\in\naturalNumbers$が存在して$A\subset \bigcup_{n=1}^N \operatorname{Int}(2E_n)$となる。
                $\bigcup_{n=1}^N \operatorname{Int}(2E_n)$を包含する十分大きな直方体$B$を考え，$2E_n\;(n=1,\dotsc,N)$を非交差的になるように適当に分割した直方体群を全て要素に持つような$B$の分割$\Delta$に対して$S(A,\Delta) \leq 2|R| <\varepsilon$となるから$\mJo{A}<\varepsilon$となる。
            \end{proof}
        ...
        \subsection{Lebesgue測度の\texorpdfstring{$\sigma$}{σ}--加法性}
            ...
            \begin{proof}
                $B_n \coloneq \bigsqcup_{i=1}^n A_i,B \coloneq \bigsqcup_{i=1}^\infty A_i$とする。
    ...
    \section{包除原理}
        \begin{shadebox}
            ...
            \end{equation}
            もっと短く書けば
            \[ \func{\mu}{\bigcup_{i=1}^n A_i} = \sum_{I\in 2^\numset\setminus\braces*{\emptyset}} (-1)^{|I|-1}\func{\mu}{\bigcap_{i\in I}A_i} \]
        \end{shadebox}
        \begin{proof}
            式(1)を証明する。
            \begin{align*}
                ...
                &= \LebInteg{S}{\bracks*{\sum_{k=1}^n (-1)^{k-1} \sum_{I\in\combi{\numset}{k}} \ind{\bigcap_{i\in I}A_i}{x}}}{\mu}{x} \\
                &= \LebInteg{S}{c(x)}{\mu}{x} \quad\text{where}\quad c(x) \coloneq \sum_{k=1}^n (-1)^{k-1} \sum_{I\in\combi{\numset}{k}} \ind{\bigcap_{i\in I}A_i}{x}
            \end{align*}
            $U\coloneq\bigcup_{i=1}^n A_i$とすると$c(x)=\ind{U}{x}$となることを示せばよい。
