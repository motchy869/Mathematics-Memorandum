\chapter{級数}
    \section{\texorpdfstring{$S_l = \sum_{k=0}^\infty k^l r^k$}{A\_l = Σ\_\{k=0\}\string^∞ k\string^l r\string^k}を逐次的に求める}
        \begin{shadebox}
            $S_l \coloneq \sum_{k=0}^\infty k^l r^k$とすると次式が成り立つ。
            \[ S_l = \frac{1}{1-r}\sum_{m=0}^{l-1}(-1)^{l-m+1}\combi{l}{m}(S_m-0^m)\]
        \end{shadebox}
        \begin{proof}
            $rS_l = \sum_{k=0}^\infty k^l r^{k+1} = \sum_{k=1}^\infty (k-1)^l r^k$より
            \begin{align*}
                (1-r)S_l &= \sum_{k=1}^\infty \bracks*{ k^l-(k-1)^l }r^k = \sum_{k=1}^{\infty}\sum_{m=0}^{l-1}\combi{l}{m}k^m(-1)^{l-m+1}r^k = \sum_{m=0}^{l-1}\combi{l}{m}(-1)^{l-m+1}\sum_{k=1}^{\infty}k^m r^k \\
    ...
    \section{Abelの総和公式: \texorpdfstring{$\sum_{i=1}^n a_ib_i = A_nb_n - \sum_{i=1}^{n-1}A_i(b_{i+1}-b_i)$}{Σ\_\{i=1\}\string^n a\_i b\_i = A\_n b\_n - Σ\_\{i=1\}\string^(n-1) A\_i(b\_(i+1) - b\_i)}}
        \begin{shadebox}
            ...
            \[ \sum_{i=1}^n a_ib_i = A_nb_n - \sum_{i=1}^{n-1}A_i(b_{i+1}-b_i) \quad \parens*{ A_i \coloneq \sum_{j=1}^i a_j } \]
        \end{shadebox}
        \begin{proof}
            (泥臭い証明)\\
            ...
            \newline
            (エレガントな証明)\\
            \quad $A_i \coloneq \sum_{j=1}^i a_j,A_0 \coloneq 0$と定義すると
            \begin{align*}
            ...
            \end{align*}
        \end{proof}
    \section{Chebyshevの和の不等式}
        \label{subsec:Chebyshevの和の不等式}
        \begin{shadebox}
            \begin{enumerate}
                \item 単調増加あるいは単調減少な数列$\{a_n\},\{b_n\}$について，$\parens*{\frac{1}{n}\sum_{i=1}^n a_i}\parens*{\frac{1}{n}\sum_{i=1}^n b_i} \leq \frac{1}{n}\sum_{i=1}^n a_ib_i$が成り立つ。
                \item 単調増加列$\{a_n\}$と単調減少列$\{b_n\}$について，$\parens*{\frac{1}{n}\sum_{i=1}^n a_i}\parens*{\frac{1}{n}\sum_{i=1}^n b_i} \geq \frac{1}{n}\sum_{i=1}^n a_ib_i$が成り立つ。
            \end{enumerate}
        \end{shadebox}
        \begin{proof}
            以下の証明は\url{https://en.wikipedia.org/wiki/Chebyshev%27s_sum_inequality}によるものである。
            \par
            1.を示す。2.は同様にして示せる。
            $S := \sum_{i=1}^n \sum_{j=1}^n (a_i-a_j)(b_i-b_j)$とする。
            $\{a_n\},\{b_n\}$の単調性の仮定から，$(a_i-a_j)$と$(b_i-b_j)$の符号は等しく，$S\geq 0$である。
            よって次式が成り立つ。
            \begin{gather*}
                0 \leq S = \sum_{i=1}^n \sum_{j=1}^n (a_i b_i - a_i b_j - a_j b_i + a_j b_j) = 2n\sum_{i=1}^n a_i b_i - 2\parens*{\sum_{i=1}^n a_i}\parens*{\sum_{j=1}^n b_j} \\
                \therefore \parens*{\frac{1}{n}\sum_{i=1}^n a_i}\parens*{\frac{1}{n}\sum_{i=1}^n b_i} \leq \frac{1}{n}\sum_{i=1}^n a_ib_i
            \end{gather*}
        \end{proof}
    \section{等比級数と0収束列の畳み込み: {\normalsize \texorpdfstring{$|r|<1,\lim_{n\to\infty}a_n=0 \Rightarrow \lim_{t\to\infty}\sum_{\tau=0}^t{r^{t-\tau}a_\tau}=0$}{|r|<1, lim\_\{n→∞\}a\_n=0 ⇒ lim\_\{t→∞\} Σ\_\{τ=0\}\string^t r\string^(t-τ) a\_τ} = 0}}
        ...
        \begin{proof}
            ...
            そこで$t\geq T$とすると
            \begin{align*}
                \abs{\sum_{\tau=0}^t{r^{t-\tau}a_\tau}} &\leq \abs{\sum_{\tau=0}^{T-1}{r^{t-\tau}a_\tau}} + \abs{\sum_{\tau=T}^t{r^{t-\tau}a_\tau}} \leq \sum_{\tau=0}^{T-1}|r|^{t-\tau}|a_\tau| + \sum_{\tau=T}^{t}|r|^{t-\tau}|a_\tau| \\
            ...
            \[\abs{\sum_{\tau=0}^t{r^{t-\tau}a_\tau}} \leq \frac{2\varepsilon}{1-|r|}\]
        \end{proof}
    \section{\texorpdfstring{$\sum_{k=1}^n \cos kx = -\frac{1}{2} + \frac{\sin(n+1/2)x}{2\sin(x/2)} \quad (x\neq 0)$}{Σ\_\{k=1\}\string^n cos(kx) = -1/2 + (sin(n+1/2)x)/(2sin(x/2))}}
        \begin{proof}
            \begin{align*}
                ...
                &= \frac{1}{2}\parens*{\sum_{l=2}^{n-1} (\sin lx + \sin(-lx)) + \sin 0x + \sin(-x) + \sin nx + \sin(n+1)x} \\
    ...
    \section{\texorpdfstring{$\sum_{k=1}^n \sin kx = \frac{\sin\frac{nx}{2}\sin\frac{(n+1)x}{2}}{2\sin(x/2)} \quad (x\neq 0)$}{Σ\_\{k=1\}\string^n sin(kx) = (sin(nx/2)sin((n+1)x/2))/(2sin(x/2))}}
        \begin{proof}
            \begin{align*}
                ...
                &= \frac{1}{2}\parens*{\sum_{l=2}^{n-1}(\cos lx - \cos(-lx)) + \cos 0x + \cos x - \cos nx - \cos (n+1)x } \\
                &= \frac{1}{2}\parens*{1 + \cos x - 2\cos\frac{(2n+1)x}{2}\cos\frac{-x}{2}} \\
                \therefore\; \sum_{k=1}^n \sin kx &= \frac{1}{2\sin x}\parens*{2\cos^2\frac{x}{2} - 2\cos\frac{(2n+1)x}{2}\cos\frac{-x}{2}} \\
                &= \frac{1}{2\sin\frac{x}{2}\cos\frac{x}{2}}\parens*{\cos^2\frac{x}{2} - \cos\frac{(2n+1)x}{2}\cos\frac{x}{2}} \\
                &= \frac{1}{2\sin\frac{x}{2}}\parens*{\cos\frac{x}{2} - \cos\frac{(2n+1)x}{2}} \\
