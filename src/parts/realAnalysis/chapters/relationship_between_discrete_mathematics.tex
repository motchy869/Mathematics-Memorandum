\chapter{離散数学との関係}
    \section{DirichletのDiophantine近似定理}
        ...
        \end{proof}
    \section{床関数と天井関数}
        \subsection{\texorpdfstring{$\forall x\in\realNumbers,\floor{-x} = -\ceil{x},\ceil{-x} = -\floor{x}$}{∀x∈R, floor(-x) = -ceil(x), ceil(-x) = -floor(x)}}
            \begin{proof}
                \quad\par
                前者は次のようにして示せる。
        ...
        \subsection{\texorpdfstring{$^\forall x,y\in\realNumbers,\floor{x}+\floor{y}\leq\floor{x+y},\ceil{x}+\ceil{y}\geq\ceil{x+y}$}{∀x,y∈R, floor(x)+floor(y) ≦ floor(x+y), ceil(x)+ceil(y) ≧ ceil(x+y)}}
            \begin{proof}
                ...
                $x=X+m_x,y=Y+m_y,X,Y\in\integers,m_x,m_y\in[0,1)$とすると$\floor{x}+\floor{y} = X+Y$であり，
                一方で$\floor{x+y} = \floor{X+Y+m_x+m_y} = X+Y + \floor{m_x+m_y} \geq X+Y$であるので，主張が従う。\\
                \newline
                ($\ceil{x}+\ceil{y}\geq\ceil{x+y}$の証明)\\
                $x=X\textcolor{red}{-}m_x,y=Y\textcolor{red}{-}m_y,X,Y\in\integers,m_x,m_y\in[0,1)$とすると$\ceil{x}+\ceil{y} = X+Y$であり，
