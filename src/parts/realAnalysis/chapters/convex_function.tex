\chapter{凸関数}
    \section{諸定理}
        ...
        \begin{proof}
            $g(\vx) \coloneq \max\{f_1(\vx), f_2(\vx)\}$とする。
            $g$の定義域内の任意の$\vx_1,\vx_2$と任意の$\theta\in[0,1]$に対して
            \[ (1-\theta)g(\vx_1) + \theta g(\vx_2) = (1-\theta)\max\{f_1(\vx_1), f_2(\vx_1)\} + \theta\max\{f_1(\vx_2), f_2(\vx_2)\} \]
            であるが，maxの性質から
            \[ \max\{f_1(\vx_1), f_2(\vx_1)\} \geq f_1(\vx_1), f_2(\vx_1) \text{かつ} \max\{f_1(\vx_2), f_2(\vx_2)\} \geq f_1(\vx_2), f_2(\vx_2) \]
            であるので
            \[(1-\theta)g(\vx_1) + \theta g(\vx_2) \geq (1-\theta)f_1(\vx_1) + \theta f_1(\vx_2) \geq f_1[(1-\theta)\vx_1 + \theta\vx_2]\]
            かつ
            \[(1-\theta)g(\vx_1) + \theta g(\vx_2) \geq (1-\theta)f_2(\vx_1) + \theta f_2(\vx_2) \geq f_2[(1-\theta)\vx_1 + \theta\vx_2]\]
            である。このことから
            \[(1-\theta)g(\vx_1) + \theta g(\vx_2) \geq \max\{f_1[(1-\theta)\vx_1 + \theta\vx_2], f_2[(1-\theta)\vx_1 + \theta\vx_2]\} = g[(1-\theta)\vx_1 + \theta\vx_2]\]
        \end{proof}
        \subsection{Jensenの不等式}
            \begin{shadebox}
                $f(x)$を凸関数とし，$w_i\geq 0(i=1,\dotsc,n),\sum_{i=1}^n w_i=1$とするとき，次式が成り立つ。
                \[ f\parens*{\sum_{i=1}^n w_i x_i} \leq \sum_{i=1}^n w_i f(x_i) \]
            \end{shadebox}
            \begin{proof}
                $n=1$の時は明らか。
                ...
                \begin{align*}
                    f\parens*{\sum_{i=1}^{n_0}w'_i x_i} &\leq \sum_{i=1}^{n_0}w'_i f(x_i) \\
                    \therefore\; f\parens*{\sum_{i=1}^{n_0}\frac{w_i}{1-w_{n_0+1}} x_i} &\leq \sum_{i=1}^{n_0}\frac{w_i}{1-w_{n_0+1}} f(x_i) \\
                    \therefore\; (1-w_{n_0+1})f\parens*{\sum_{i=1}^{n_0}\frac{w_i}{1-w_{n_0+1}} x_i} &\leq \sum_{i=1}^{n_0} w_i f(x_i) \\
                    \therefore\; w_{n_0+1}f(x_{n_0+1}) + (1-w_{n_0+1})f\parens*{X_{n_0}} &\leq \sum_{i=1}^{n_0+1} w_i f(x_i) \quad \parens*{X_{n_0} = \sum_{i=1}^{n_0}\frac{w_i}{1-w_{n_0+1}} x_i \text{とおいた}}
                \end{align*}
                $f$の凸性より
                \[ \text{左辺} \geq f(w_{n_0+1}x_{n_0+1} + (1-w_{n_0+1})X_{n_0}) = f\parens*{\sum_{i=1}^{n_0+1}w_i x_i}\]
            \end{proof}
        ...
        \subsection{2階導関数と凸性の関係}
            ...
            \begin{proof}
                ...
                $1\Rightarrow 2$を示す。
                $x\in I,x\neq x_1$とする。
                $g(x) \coloneq f(x) - \parens*{f(x_1) - f^{(1)}(x_1)(x-x_1)}$とすると$g^{(1)}(x) = f^{(1)}(x) - f^{(1)}(x_1)$なので$g^{(1)}(x_1) = 0$である。
                ...
                \begin{align*}
                    ...
                    &= f(x_3) + f^{(1)}(x_3)\bracks*{(1-\lambda)(x_1-x_3) + \lambda(x_2-x_3)} \\
                    &= f(x_3) + f^{(1)}(x_3)\bracks*{(1-\lambda)x_1 + \lambda x_2 - x_3} \\
    ...
    \section{Legendre変換}
        \subsection{十分滑らかな狭義凸関数に対してLegendre変換を2回施すと元の関数に戻る}
            \begin{proof}
                ...
                \[ f^*(p) = p\fd^{-1}(p) - f\parens*{\fd^{-1}(p)} \quad (p \in I_2) \]
                $f^*$のLegendre変換$f^{**}$を求めるためにまず${f^*}^{(1)}$の値域を調べる。
                \[ \fStarD(q)\;(q \in I_2) = \fd^{-1}(q) + q\frac{1}{f^{(2)}(q)} - \fd\parens*{\fd^{-1}(q)}\frac{1}{f^{(2)}(q)} = \fd^{-1}(q) \]
                より$\fStarD$の値域は$I$である。
                さらに，$\fd^{-1}$は狭義単調増加なので上式より$\fStarD$の逆関数が存在して$\fStarD^{-1} = \fd$である。
                $\fStarD^{-1}$の定義域は$I$である。
                以上より$f^{**}$は次のようになる。
                \begin{align*}
                    f^{**}(q)\; (q \in I) &= q\fStarD^{-1}(q) - f^*\parens*{\fStarD^{-1}(q)} \\
                    &= q\fd(q) - f^*(\fd) \\
                    &= q\fd(q) - \fd(q)\fd^{-1}\parens*{\fd(q)} + f\parens*{\fd^{-1}\parens*{\fd(q)}} \\
                    &= f(q)
                \end{align*}
            \end{proof}
            \let\fd\undefined
