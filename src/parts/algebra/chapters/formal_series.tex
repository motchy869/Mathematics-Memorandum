    ...
    \section{原始多項式との関係}
        \subsection{原始多項式の逆数の係数列は周期的}
            ...
            \begin{proof}
                ...
                \begin{align*}
                    x^{p^n-1} - 1 &= g(x)f(x) \\
                    \therefore f(x)^{-1} &= -g(x)\parens*{1 - x^{p^n-1}}^{-1} = -g(x) \sum_{i=0}^\infty x^{i(p^n-1)}
                \end{align*}
        ...
        \subsection{原始多項式\texorpdfstring{$f_1(x)$}{f1(x)}の係数の順番を逆転した\texorpdfstring{$f_2(x)$}{f2(x)}の逆数の係数列は\texorpdfstring{$f_1(x)^{-1}$}{1/f1(x)}の係数列の逆順逆符号}
            ...
            \subsubsection{証明}
                \begin{proof}
                    ...
                    \begin{align*}
                        f_1(x)^{-1} &= g_1(x) \sum_{i=0}^\infty x^{i(p^n-1)} = -g_1(x) \parens*{x^{p^n-1} - 1}^{-1} \\
                    ...
                    これより次式を得る。
                    \[ f_2(x)^{-1} = -g_2(x) \parens*{1 - x^{p^n-1}}^{-1} = -g_2(x) \sum_{i=0}^\infty x^{i(p^n-1)} \]
                \end{proof}
