\chapter{行列の階数}
    ...
    \section{\texorpdfstring{$A\in\field^{m\times n}$}{A∈F\string^(m×n)}の階数が\texorpdfstring{$r$}{r}であるなら，ある正則行列\texorpdfstring{$T_x\in\field^{n\times n},\quad T_y\in\field^{m\times m}$}{Tx∈\string^(n×n), Ty∈\string^(m×m)}が存在して...}
        ...
        \begin{proof}
            ...
            である。$\bm{y}_i\coloneq A\bm{x}_i\quad(i=1,2,\dotsc,r)$とし，これらを含む$\field^m$の基底を$\{\bm{y}_1,\bm{y}_2,\dotsc,\bm{y}_r,\bm{y}_{r+1},\dotsc,\bm{y}_m\}$とすると
            $T_x\coloneq[\bm{x}_1,\bm{x}_2,\dotsc,\bm{x}_n],\quad T_y\coloneq[\bm{y}_1,\bm{y}_2,\dotsc,\bm{y}_m]$は正則であり，
            \begin{align*}
