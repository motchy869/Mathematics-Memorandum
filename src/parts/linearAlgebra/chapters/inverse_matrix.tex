\chapter{逆行列}
    ...
    \section{(系)結合係数の抽出}
        \begin{shadebox}
            ...
            このとき，任意に与えられたベクトル$\vx = \sum_{i=1}^{n}{c_i\phi_i}\in\field^n$ ($\bm{\phi}_1,\dotsc,\bm{\phi}_n$は$\field^n$の基底だから$\vx$は必ずこのように分解できる!)から$c_1,\dotsc c_n$を抽出するには左から$[\bm{\phi}_1,\dotsc,\bm{\phi}_n]^{-1}$を掛ければよい。
            詳しく言うと，次式が成り立つ。
            \[[c_1,\dots,c_n]^\top = [\bm{\phi}_1,\dotsc,\bm{\phi}_n]^{-1}\vx\]
        \end{shadebox}
    ...
    \section{正則行列\texorpdfstring{$A$}{A}の列(行)が直交系を成すとき，\texorpdfstring{$A^{-1}$}{Aの逆行列}の行(列)は直交系を成す}
        ...
        \begin{proof}
            $A$の列が直交系を成すとする。
            $A^{-1}\parens*{A^{-1}}^\top = A^{-1}\parens*{A^\top}^{-1} = \parens*{A^\top A}^{-1}$であるが，仮定より$A^\top A$が正則な対角行列であるので$\parens*{A^\top A}^{-1}$は対角行列になり，定理の主張が成り立つ。
    ...
    \section{列(or行)を入れ替えた行列の逆行列}
        ...
        \end{proof}
    \section{Sherman-Morrisonの公式の特別な場合\texorpdfstring{: $(I+\bm{u}\HerConj{\vv})^{-1} = (I-\bm{u}\HerConj{\vv}/(1+\HerConj{\vv}\bm{u}))$}{}}
        \label{Sherman-Morrisonの公式の特別な場合}
        \begin{shadebox}
            $\bm{u},\vv \in \complexNumbers^m, 1+\HerConj{\vv}\bm{u} \neq 0$とするとき次式が成り立つ。
            \[ (I+\bm{u}\HerConj{\vv})^{-1} = I - \frac{1}{1+\HerConj{\vv}\bm{u}}\bm{u}\HerConj{\vv} \]
        \end{shadebox}
        \begin{proof}
            Woodburyの公式の特別な場合として片付けるのが一般的だが，ここではJordan分解を用いて導出してみる。
            $\bm{u}\HerConj{\vv} = O$のときは定理の主張は明らかに成り立つ。
            以下では$\bm{u}\HerConj{\vv} \neq O$とする。
            仮定$1+\HerConj{\vv}\bm{u} \neq 0$と\ref{I-ABの行列式}より$I+\bm{u}\HerConj{\vv}$には逆行列が存在する。
            $\bm{u}\HerConj{\vv}$の階数は1であり($\img{\bm{u}\HerConj{\vv}} = \Span{\bm{u}}$)，唯一の固有値は$\HerConj{\vv}\bm{u}$である(対応する固有ベクトルは$\bm{u}$)。
            よって$\bm{u}\HerConj{\vv}$をJordan分解すると，適当な正則行列$J$を用いて次のように表せる。
            \[
                \bm{u}\HerConj{\vv} =
                \begin{bmatrix}
                    \HerConj{\vv}\bm{u} & \bm{0}^\top \\
                    \bm{0} & O
                \end{bmatrix}
                =: J\Lambda J^{-1}
            \]
            よって
            \begin{align*}
                (I+\bm{u}\HerConj{\vv})^{-1} &= J(I + \Lambda)^{-1}J^{-1} = J
                \begin{bmatrix}
                    1+\HerConj{\vv}\bm{u} & \bm{0}^\top \\
                    \bm{0} & I
                \end{bmatrix}
                ^{-1}J^{-1} = J
                \begin{bmatrix}
                    \frac{1}{1+\HerConj{\vv}\bm{u}} & \bm{0}^\top \\
                    \bm{0} & I
                \end{bmatrix}
                J^{-1} \\
                &= J\frac{1}{1+\HerConj{\vv}\bm{u}}
                \begin{bmatrix}
                    1 & \bm{0}^\top \\
                    \bm{0} & (1+\HerConj{\vv}\bm{u})I
                \end{bmatrix}
                J^{-1} = J\frac{1}{1+\HerConj{\vv}\bm{u}}((1+\HerConj{\vv}\bm{u})I - \Lambda)J^{-1} \\
                &= I - \frac{1}{1+\HerConj{\vv}\bm{u}}\bm{u}\HerConj{\vv}
            \end{align*}
        \end{proof}
    \section{Sherman-Morrisonの公式\texorpdfstring{: $(A+\bm{u}\HerConj{\vv})^{-1} = (I-A^{-1}\bm{u}\HerConj{\vv}/(1+\HerConj{\vv}A^{-1}\bm{u}))A^{-1}$}{}}
        \begin{shadebox}
            $A \in \complexNumbers^{m\times m}, \bm{u},\vv \in \complexNumbers^m$とする。
            $A$は可逆であるとし，$1+\HerConj{\vv}A^{-1}\bm{u} \neq 0$とする。
            このとき次式が成り立つ。
            \[ (A+\bm{u}\HerConj{\vv})^{-1} = \parens*{ I - \frac{1}{1+\HerConj{\vv}A^{-1}\bm{u}}A^{-1}\bm{u}\HerConj{\vv} }A^{-1} \]
        \end{shadebox}
        \begin{proof}
            $A+\bm{u}\HerConj{\vv} = A(I + A^{-1}\bm{u}\HerConj{\vv})$だから$(A+\bm{u}\HerConj{\vv})^{-1} = (I + A^{-1}\bm{u}\HerConj{\vv})^{-1}A^{-1}$であり，2つ目の因子に\ref{Sherman-Morrisonの公式の特別な場合}を適用すればよい。
        \end{proof}
    \section{Woodburyの公式の特別な場合\texorpdfstring{: $(I_n+UV)^{-1} = I_n-U(I_k+VU)^{-1}V$}{}}
        ...
        \begin{proof}
            ...
            \end{align*}
        \end{proof}
    \section{Woodburyの公式\texorpdfstring{: $(A+UCV)^{-1} = \bracks*{I_n - A^{-1}U(C^{-1} + VA^{-1}U)^{-1}V}A^{-1}$}{}}
        \begin{shadebox}
            $I_n$を$n$次の単位行列とする。
            ...
            このとき次式が成り立つ。
            \[ (A+UCV)^{-1} = \bracks*{I_n - A^{-1}U(C^{-1} + VA^{-1}U)^{-1}V}A^{-1} \]
        \end{shadebox}
        \begin{proof}
            $A+UCV = A(I_n + A^{-1}UCV)$であるから$(A+UCV)^{-1} = (I_n + A^{-1}UCV)^{-1}A^{-1}$である。
            \ref{Woodburyの公式の特別な場合}で$U$を$A^{-1}U$で，$V$を$CV$で置き換えて上式に適用すると次式を得る。
            \[ (A+UCV)^{-1} = \bracks*{I_n - A^{-1}U(I_k + CVA^{-1}U)^{-1}CV}A^{-1} = \bracks*{I_n - A^{-1}U(C^{-1} + VA^{-1}U)^{-1}V}A^{-1} \]
        \end{proof}