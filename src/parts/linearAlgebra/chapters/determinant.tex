\chapter{行列式}
    ...
    \section{\texorpdfstring{$A\in\realNumbers^{n\times m}, B\in\realNumbers^{m\times n}$}{A∈R\string^(n×m), B∈R\string^(m×n)}に対して\texorpdfstring{$|I_m-AB| = |I_n-BA|$}{|Im-AB| = |In-BA|}}
        ...
        \begin{proof}
            ...
            \]
        \end{proof}
    \section{系: \texorpdfstring{$\left|I_n-\vv\vv^\top\right| = 1-\norm{\vv}_2^2$}{|In-vv\string^| = 1 - ||v||\string^2}}
        \begin{shadebox}
            $\vv \in \complexNumbers^n$に対して$\left|I_n-\vv\vv^\top\right| = 1-\norm{\vv}_2^2$
        \end{shadebox}
    \section{逆対角転置行列の行列式は元の行列のそれと等しい}
        \begin{proof}
            ...
            \begin{align*}
                |A'| &= \sum_{\sigma\in S_n} \sgn\parens*{\sigma} \prod_{i=1}^n A'[i,\sigma(i)] = \sum_{\sigma\in S_n} \sgn\parens*{\sigma} \prod_{i=1}^n A[n-\sigma(i)+1, n-i+1] \\
                &= \sum_{\sigma\in S_n} \sgn\parens*{\sigma} \prod_{j=1}^n A[j, n- \sigma^{-1}(n-j+1)+1] \\
                &= \sum_{\sigma\in S_n} \sgn\parens*{\sigma} \prod_{j=1}^n A[j, \tilde\sigma(j)] \quad (\tilde\sigma: j \mapsto n+1-\sigma^{-1}(n-j+1))
            \end{align*}
            ここで逆置換の符号が元の置換の符号と等しいこと，および\ref{置換の引数を逆順にしたときの符号}, \ref{出力を逆順にしたときの置換の符号}より$\sgn\parens*{\tilde\sigma} = s^2\sgn\parens*{\sigma} = \sgn\parens*{\sigma}$であるから
            \[ |A'| = \sum_{\sigma\in S_n} \sgn\parens*{\tilde\sigma} \prod_{j=1}^n A[j, \tilde\sigma(j)] \]
            ...
            \[ |A'| = \sum_{\tilde\sigma\in S_n} \sgn\parens*{\tilde\sigma} \prod_{j=1}^n A[j, \tilde\sigma(j)] = |A| \]
        \end{proof}
