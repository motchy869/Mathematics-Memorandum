\chapter{ベクトルのノルム}
    \section{諸定理}
        ...
        % 		Lagrangeの未定乗数法を使う。
        % 		目的関数を$f(\bm{x})\coloneq\sum_{i=1}^n{x_i}$，制約条件を$g(\bm{x})\coloneq\norm{\bm{x}}_2^2-1=0$とし，Lagrange関数を$L(\bm{x},\lambda)\coloneq f(\bm{x})+\lambda g(\bm{x})$とする。
        % 		最適解を$\HerConj{\bm{x}}$とすると$\nabla_{\bm{x}}L(\bm{x},\lambda)=\bm{0}$より$\bm{1}_n+2\lambda\HerConj{\bm{x}}=\bm{0}\;\therefore\;\bm{x}=-\frac{1}{2\lambda}\bm{1}_n$。
        % 		よって$\HerConj{\bm{x}}$は全要素が等しい。これと$\norm{\HerConj{\bm{x}}}=1,\HerConj{\bm{x}}\geq\bm{0}$より$\HerConj{\bm{x}} = \frac{1}{\sqrt{n}}\bm{1}_n$であり，$f(\HerConj{\bm{x}})=\sqrt{n}$
        % 	\end{proof}
        \subsection{\texorpdfstring{$\bm{x}\in\complexNumbers^n,\norm{\bm{x}}_2=1\;\Rightarrow\;\sum_{i=1}^n{|x_i|}\leq\sqrt{n}$}{x∈C\string^n, ||x||\_2=1, ⇒ |x\_1|+...+|x\_n| ≦ √n}}
            ...
            \begin{proof}
                \quad\par
                $x_k = r_k \NapierE^{i\theta_k},\; (r_k \geq 0, \theta_k \in \realNumbers,\;k=1,\dotsc,n)$とすると$\norm{\bm{x}}_2=1 \iff \sum_{k=1}^n {r_k}^2 = 1,\;\sum_{k=1}^n{|x_k|} = \sum_{k=1}^n r_k$であるから，制約条件$g(\bm{r}) \coloneq \sum_{k=1}^n {r_k}^2 - 1 = 0,\;\bm{r} \geq \bm{0}_n$の下で関数$f(\bm{r}) \coloneq \sum_{k=1}^n r_k$の最大値が$\sqrt{n}$以下であること(*)を示せばよい。
                \par
                ...
                Lagrangeの未定乗数法を使う。
                Lagrange関数を$L(\bm{r},\lambda)\coloneq f(\bm{r})+\lambda g(\bm{r})$とする。
