\chapter{行列の演算}
    \section{結合則}
        ...
        \begin{proof}
            まず下準備。
            \[A \coloneq \bracks*{a_{ij}}_{l\times m},\quad B \coloneq \bracks*{b_{ij}}_{m\times n},\quad C \coloneq \bracks*{c_{ij}}_{n\times o}\]
            \[M_{AB} \coloneq \bracks*{m_{{AB}_{ij}}}_{l\times n} = AB,\quad M_{BC} \coloneq [m_{{BC}_{ij}}]_{m\times o} = BC\]
            \[N_1 \coloneq \bracks*{n_{1_{ij}}}_{l\times o} = (AB)C = M_{AB}C,\quad N_2 \coloneq \bracks*{n_{2_{ij}}}_{l\times o} = A(BC) = AM_{BC}\]
            とする。
            $N_1 = N_2$すなわち$n_{1_{ij}} = n_{2_{ij}}$を示せば良い。\\
            まず
            \[m_{{AB}_{ij}} = \sum_{k=1}^{m}{a_{ik}b_{kj}},\quad m_{{BC}_{ij}} = \sum_{k=1}^{n}{b_{ik}c_{kj}}\]
            である。これを用いて
            \[n_{1_{ij}} = \sum_{k=1}^{\textcolor{red}{n}}{m_{{AB}_{ik}}c_{kj}} = \sum_{k=1}^{n}{\parens*{\sum_{l=1}^{m}{a_{il}b_{lk}}}c_{kj}} = \sum_{\textcolor{red}{k}=1}^{n}{\sum_{\textcolor{red}{l}=1}^{m}{\parens*{a_{il}b_{lk}c_{kj}}}}\]
            \[n_{2_{ij}} = \sum_{k=1}^{\textcolor{red}{m}}{a_{ik}m_{{BC}_{kj}}} = \sum_{k=1}^{m}{a_{ik}\parens*{\sum_{l=1}^{n}{b_{kl}c_{lj}}}} = \sum_{\textcolor{red}{l}=1}^{n}{\sum_{\textcolor{red}{k}=1}^{m}{\parens*{a_{ik}b_{kl}c_{lj}}}}\]
            上式の$k$と$l$を交換(純粋に紙の上で文字として書き換える)しても何ら問題はない。そうすると上式は
            \[\sum_{\textcolor{red}{k}=1}^{n}{\sum_{\textcolor{red}{l}=1}^{m}{\parens*{a_{il}b_{lk}c_{kj}}}} = n_{1_{ij}}\]
    ...
    \section{上(下)三角行列のべき乗}
        ...
        \begin{proof}
            ...
            \begin{align*}
                A^{n+1} &= AA^n = \sum_{k=1}^{n}{a_{ik}b_{kj}}\quad\parens*{b_{kj} = A^n[k][j]} \\
    ...
    \section{Kronecker積}
        \subsection{諸注意}
            \subsubsection{\texorpdfstring{$m\times 1,n\times 1$}{m×1,n×1}行列のKronecker積で表せない\texorpdfstring{$(m+n)\times 1$}{(m+n)×1}行列が存在する}
            \begin{proof}
                例えば$m=n=2$のとき$[1, 2, 1, 3]^\top$は$\complexNumbers^2$の元のKronecker積で表せない。
            \end{proof}
            （補足）
            \par
            $\complexNumbers^{m\times n}$と$\complexNumbers^{m+n}$の間に一対一の対応が存在すること，および$\bm{a}\in\complexNumbers^m,\bm{b}\in\complexNumbers^n$に対して$\bm{a}\bm{b}^\top$の階数が高々1であることからも直ちに判る。
        ...
        \subsection{固有値、固有ベクトル}
            ...
            \begin{shadebox}
                ...
                $\lambda_{A,i}$に属する$A$の固有ベクトルを$\vv_{A,i}$，$\lambda_{B,i}$に属する$B$の固有ベクトルを$\vv_{B,i}$とする。
                $A\otimes B$の固有値は重複を含めて$\lambda_{A,i}\lambda_{B,j}\;(i\in\range{1}{m_A},j\in\range{1}{m_B})$である。
                $\vv_{A,i}\otimes\vv_{A,j}$は$A\otimes B$の固有値$\lambda_{A,i}\lambda_{B,j}$に属する固有ベクトルである。
            \end{shadebox}
            \begin{proof}
                $A,B$のSchur分解をそれぞれ$A = S_A U_A \HerConj{S_A}, B = S_B U_B \HerConj{S_B}$とする。
                ...
                \par
                混合積の性質から$(A\otimes B)(\vv_{A,i}\otimes\vv_{B,j}) = (A\vv_{A,i})\otimes(B\vv_{B,j}) = \lambda_{A,i}\vv_{A,i}\otimes\lambda_{B,j}\vv_{B,j} = \lambda_{A,i}\lambda_{B,j}(\vv_{A,i}\otimes\vv_{B,j})$であるから固有ベクトルについても定理の主張が成り立つ。
            \end{proof}
        \subsection{行列式}
            \begin{shadebox}
