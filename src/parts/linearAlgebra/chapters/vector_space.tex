        ...
        \subsection{諸定理}
            \subsubsection{三角関数ベクトルによる直交展開}
                \begin{shadebox}
                    ...
                    \[\left\{\vv_1,\dotsc,\vv_n \left| \vv_k[l] = \frac{1}{\sqrt{n}}\exp\left(i\frac{k}{n}2\pi l\right) \right. \right\}\]
                    は$\complexNumbers^n$の正規直交基底を成す。
                \end{shadebox}
                \begin{proof}
                    \quad\par
                    まず各ベクトルの2ノルムが1であることが次のようにして簡単に確かめられる。
                    \[\norm{\vv_k}_2 = \frac{1}{\sqrt{n}}\sqrt{\sum_{l=1}^n{\abs{\exp\left(i\frac{k}{n}2\pi l\right)}^2}} = \frac{1}{\sqrt{n}}\sqrt{n} = 1\]
                    次に，相異なる$k,l$に対して$\vv_k$と$\vv_l$が直交することは次のようにして確かめられる。
                    \[\inProd{\vv_k}{\vv_l} = \HerConj{{\vv_k}}\vv_l = \frac{1}{n}\sum_{m=1}^n{\exp\left(-i\frac{k}{n}2\pi m\right)\exp\left(i\frac{l}{n}2\pi m\right)} = \frac{1}{n}\sum_{m=1}^n{\exp\left(i\frac{l-k}{n}2\pi m\right)} = 0\quad(\because\;\ref{fml:花びら定理})\]
                \end{proof}
