\chapter{固有値,固有ベクトル}
    \section{諸定理}
        ...
        \subsection{ユニタリ行列の全ての固有値の絶対値は1である}
            \begin{proof}
                \quad\par
                ユニタリ行列$P$とその任意の固有値$\lambda$およびそれに対応する固有ベクトル$\vx \neq \bm{0}$について$P\vx = \lambda\vx$の両辺のノルムをとると$\|\vx\|_2 = \|P\vx\|_2 = |\lambda|\|\vx\|_2$であるからわかる。
            \end{proof}
        \subsection{随伴行列}
            \subsubsection{定義}
                \label{随伴行列の定義}
    ...
    \section{一般固有空間}
        \subsection{一般固有空間の階数の頭打ち}
            ...
            \begin{proof}
                ...
                すなわち$(A-\lambda I)^{l+1}\left[(A-\lambda I)\vv\right]=\bm{0}\land(A-\lambda I)^l\left[(A-\lambda I)\vv\right]\neq\bm{0}$。
            ...
            \begin{shadebox}
                ...
                \begin{proof}
                    ...
                    \[
                        ...
                        \end{bmatrix}
                        ,\quad
                        \vv = \frac{-1}{\lambda}J(\lambda,n)^{-1}\bm{b} = \left[\frac{-1}{\lambda}J(\lambda,n)^{-1}[i][n]\right]_{n\times 1} = \left[\frac{(-1)^{(n+1)-i}}{\lambda^{(n+1)-i+1}}\right]_{n\times 1}
                    \]
