\chapter{固有値,固有ベクトル}
    \section{諸定理}
        ...
        \subsection{相異なる固有値に対応する固有ベクトルは一次独立}
            ...
            \begin{shadebox}
                $A\in\complexNumbers^{n\times n}$の相異なる$p$個の固有値$\lambda_i\;(i=1,\dotsc,p)$に対応する固有ベクトルを$\vv_i$とすると$\vv_1,\dotsc,\vv_p$は一次独立である。
            \end{shadebox}
            \begin{proof}
                $c_1,\dotsc,c_p$を定数として
                \[c_1\vv_1 + \dotsb + c_p\vv_p = \bm{0}_n\]
                とおいて両辺に左から$A$を掛けると
                \[\lambda_1 c_1\vv_1 + \dotsb + \lambda_p c_p\vv_p = \bm{0}_n\]
                となる。これを繰り返して
                \begin{align*}
                    {\lambda_1}^2 c_1\vv_1 + \dotsb + {\lambda_p}^2 c_p\vv_p &= \bm{0}_n \\
                    & \vdots \\
                    {\lambda_1}^{p-1} c_1\vv_1 + \dotsb + {\lambda_p}^{p-1} c_p\vv_p &= \bm{0}_n
                \end{align*}
                を得る。これより
                \[
                    [c_1\vv_1,\dotsc,c_p\vv_p]
                    \begin{bmatrix}
                    ...
                    = O_{n\times p}
                \]
                となるが，$\lambda_i\;(i=1,\dotsc,p)$が相異なることより，上式の等号の左隣のVandermonde行列は正則だから
                \[ [c_1\vv_1,\dotsc,c_p\vv_p] = O_{n\times p} \]
        ...
        \subsection{\texorpdfstring{$n\times n$}{n×n}行列\texorpdfstring{$A$}{A}の全要素が\texorpdfstring{$a\neq 0$}{a≠0}であるなら非零固有値は\texorpdfstring{$an$}{an}のみ}
            \begin{proof}
                ...
                \[
                    ...
                    \end{cases}
                \]
                $\{\bm{b}_1,\dotsc,\bm{b}_n\}$が一次独立であることを示せば，$\abs{\lambda I-A}\neq 0$が示せて，$\lambda$が固有値でないことが示せる。
        ...
        \subsection{逆行列の固有値,固有ベクトル}
            ...
            \begin{proof}
                ...
                $A^{-1}$の固有値の一つを$\lambda'\neq0$とし，それに対する固有ベクトルを$\vy\neq\bm{0}$とおくと
                $A^{-1}\vy = \lambda'\vy$であり，両辺に左から$A$を掛けて$\vy = \lambda'A\vy\quad\therefore\;A\vy = \frac{1}{\lambda'}\vy$となる。
                よって$A^{-1}$の固有値$\lambda'$は$A$の固有値$1/\lambda'$の逆数であり，固有ベクトルは共通である。
            \end{proof}
        \subsection{ユニタリ行列の全ての固有値の絶対値は1である}
            \begin{proof}
                ユニタリ行列$P$とその任意の固有値$\lambda$およびそれに対応する固有ベクトル$\vx \neq \bm{0}$について$P\vx = \lambda\vx$の両辺のノルムをとると$\norm{\vx}_2 = \norm{P\vx}_2 = |\lambda|\norm{\vx}_2$であるからわかる。
            \end{proof}
        \subsection{随伴行列}
            \subsubsection{定義}
                \label{随伴行列の定義}
            ...
            \subsubsection{固有ベクトル}
                ...
                \begin{shadebox}
                    \ref{随伴行列の定義}の定義を引き継ぐ。
                    $\lambda$は$A$の固有値であるとする\footnote{$\lambda=0$も含む。何故ならば$\vv\neq\bm{0}$が必ず成り立つから。}。
                    $\vv\coloneq[\lambda^{n-1},\lambda^{n-2},\dots,\lambda,1]^\top$は$A$の固有ベクトルである。
                \end{shadebox}
                \begin{proof}
                    \ref{随伴行列の行列式と固有多項式}より$-a_{n-1}\lambda^{n-1}- a_{n-2}\lambda^{n-2} - \dots - a_1\lambda - a_0 = \lambda^n$であるから，次式が成り立つ。
                    \[ A\vv = [\lambda^n,\lambda^{n-1},\dots,\lambda^2,\lambda] = \lambda\vv \]
                \end{proof}
            \subsubsection{対角化}
                \begin{shadebox}
                    \ref{随伴行列の定義}の定義を引き継ぐ。
                    $A$の固有値が全て異なるとし，それらを$\lambda_i\;(i=0,1,\dots,n)$とする。
                    $\lambda_i$に対応する固有ベクトルを$\vv_i$とする。
                    $A$は$\lambda_i\;(i=0,1,\dots,n)$に関するVandermonde行列$V$を用いて次のように対角化できる。
                    \[
        ...
        \subsection{巡回行列の固有値}
            \begin{shadebox}
                ...
                \[
                    \vecEntry{\bm{a}_m}{i}\coloneq\vecEntry{\bm{a}}{(i-m)\%N}
                \]
                ...
                $\DFT{\bm{a}}$は$\bm{a}$のDFT(離散Fourier変換)であり，次式で定義される。
                \[ \DFT{\bm{a}} \coloneq \HerConj{W}\bm{a} \]
                すなわち第$k$要素は次式である。
                \[ \sum_{n=0}^{N-1}\conj{W_{k,n}}a_n \]
            \end{shadebox}
            \begin{proof}
        ...
        \subsection{Hadamard行列の固有値}
            ...
            \begin{proof}
                ...
                $H_m$の固有値$2^{m/2}$に対応する相異なる固有ベクトルを$\bm{u}_m^1,\dotsc,\bm{u}_m^{2^{m-1}}$とし，固有値$-2^{m/2}$に対応する相異なる固有ベクトルを$\vv_m^1,\dotsc,\vv_m^{2^{m-1}}$とする。
                このとき次のことが成り立つ。\\
    ...
    \section{スペクトル写像定理}
        ...
        \begin{proof}
            ...
            $f(A)$の固有多項式は
            \begin{align*}
                \abs{\lambda I - f(A)} &= \abs{\lambda I - \sum_{k=0}^{m}c_{k}A^k} = \abs{\lambda I - \sum_{k=0}^{m}c_{k}PU^kP^\top} = \abs{P\parens*{\lambda I - \sum_{k=0}^{m}c_{k}U^k}P^\top} \\
                &= \abs{\lambda I - \sum_{k=0}^{m}c_{k}U^k}
            \end{align*}
            ...
            よって上の式は
            \[\abs{\lambda I - \sum_{k=0}^{m}c_{k}U^k} = \prod_{i=1}^{n}{(\lambda - f(\lambda_i))}\]
    ...
    \section{一般固有空間}
        \subsection{一般固有空間の階数の頭打ち}
            ...
            \begin{proof}
                ...
                結論を否定すると，ある$\vv\in F_{A,m}(\lambda)$が存在して$(A-\lambda I)^{l+2}\vv=\bm{0}\land(A-\lambda I)^{l+1}\vv\neq\bm{0}$。
                すなわち$(A-\lambda I)^{l+1}\bracks*{(A-\lambda I)\vv}=\bm{0}\land(A-\lambda I)^l\bracks*{(A-\lambda I)\vv}\neq\bm{0}$。
                よって$(A-\lambda I)\vv \in F_{A,l+1}(\lambda)$。
                これと仮定$F_{A,l+1}(\lambda) = F_{A,l}(\lambda)$より$(A-\lambda I)\vv \in F_{A,l}(\lambda)$。
                よって$(A-\lambda I)^{l+1}\vv=\bm{0}$となり，矛盾する。
            \end{proof}
        ...
        \subsection{一般固有空間への直和分解}
            ...
            \begin{proof}
                ...
                まず固有値$\lambda_i$を持つことを示す。$\vx\in E_A(\lambda_i) \subseteq F_A(\lambda_i)$を任意に取り，$F_A(\lambda_i)$の基底で次のように展開する。
                \[\vx = \sum_{j=1}^{{\nu_i}'}{c_j\bm{\phi}_{ij}}\]
                これを上式に右から掛けて
                \begin{align}
                    A\vx &= \Phi\;\diag{A_1,\dotsc,A_p}\Phi^{-1}\sum_{j=1}^{{\nu_i}'}{c_j\bm{\phi}_{ij}} = \Phi\;\diag{A_1,\dotsc,A_p}\sum_{j=1}^{{\nu_i}'}{c_j\bm{e}_{j+\sum_{k=1}^{i-1}{{\nu_k}'}}}\quad(\because \mathrm{定理\ref{逆行列にその構成縦ベクトルを掛ける}}) \nonumber\\
                ...
                \[A\vx = (1) = \sum_{j=1}^{{\nu_i}'}{d_j\bm{\phi}_{ij}}\]
                であるが，そもそも$A\vx = \lambda_i\vx = \sum_{j=1}^{{\nu_i}'}{\lambda_ic_j\bm{\phi}_{ij}}$であったから$d_j = \lambda_jc_j$となり，
                $A_i[c_1,\dotsc,c_{{\nu_i}'}]^\top = \lambda_i[c_1,\dotsc,c_{{\nu_i}'}]^\top$となるから，$A_i$は固有値$\lambda_i$を持つことになる。
                \par
                次に$A_i$の固有値が$\lambda_i$のみであることを背理法で示す。
                $A_i$が固有値$\lambda'\notin\{\lambda_1,\dotsc,\lambda_p\}$を持つと仮定し，$\lambda'$に対する固有ベクトルの一つを$\vy = [y_1,\dotsc,y_{{\nu_i}'}]^\top\in\field^{{\nu_i}'}$とする($A_i\vy=\lambda'\vy$)。
                ベクトル$\vx\in\field^n$を
                \[\Phi^{-1}\vx = [\underbrace{0,\dotsc,0}_{{\nu_1}'+\dotsb+{\nu_{i-1}}'},\vy^\top,\underbrace{0,\dotsc,0}_{{\nu_{i+1}}'+\dotsb+{\nu_p}'}]^\top\]
                すなわち
                \[\vx = \Phi[\underbrace{0,\dotsc,0}_{{\nu_1}'+\dotsb+{\nu_{i-1}}'},\vy^\top,\underbrace{0,\dotsc,0}_{{\nu_{i+1}}'+\dotsb+{\nu_p}'}]^\top = \sum_{j=1}^{{\nu_i}'}{y_j\bm{\phi}_{ij}}\]
                で定めると$\bm{\phi}_{i1},\dotsc,\bm{\phi}_{i{\nu_i}'}$の和で表されているから$\vx\in F_A(\lambda_i)$であるので，まず次式が成り立つ。
                \begin{equation}
                    (A-\lambda_iI_n)^{\nu_i}\vx = \bm{0}_n
                \end{equation}
                一方で
                \begin{align*}
                    (A-\lambda_iI_n)\vx &= A\vx - \lambda_i\vx = \Phi\;\diag{A_1,\dotsc,A_p}\Phi^{-1}\vx - \lambda_i\vx\\
                    &= \Phi\;\diag{A_1,\dotsc,A_p}[\underbrace{0,\dotsc,0}_{{\nu_1}'+\dotsb+{\nu_{i-1}}'},\vy^\top,\underbrace{0,\dotsc,0}_{{\nu_{i+1}}'+\dotsb+{\nu_p}'}]^\top - \lambda_i\vx\\
                    &= \Phi\bracks*{
                        \begin{array}{c}
                            \bm{0}_{{\nu_1}'}\\
                    ...
                    &= \lambda'\Phi[\underbrace{0,\dotsc,0}_{{\nu_1}'+\dotsb+{\nu_{i-1}}'},\vy^\top,\underbrace{0,\dotsc,0}_{{\nu_{i+1}}'+\dotsb+{\nu_p}'}]^\top - \lambda_i\vx = \lambda'\vx-\lambda_i\vx\\
                    &= (\lambda'-\lambda_i)\vx
                \end{align*}
                であるから
                \[(A-\lambda_iI_n)^{\nu_i}\vx = (\lambda'-\lambda_i)^{\nu_i}\vx \neq \bm{0}_n\]
    ...
    \section{Jordan標準形}{
        ...
        \subsection{Jordanブロックの逆行列}
            \begin{shadebox}
                ...
                に対して
                \[
                    J(\lambda,n)^{-1} = \bracks*{\indII{i\leq j}\frac{(-1)^{j-i}}{\lambda^{j-i+1}}}_{n\times n} =
                    \begin{bmatrix}
                ...
                \begin{proof}
                    ...
                    \[ J(\lambda,n)^{-1} = M(\lambda)^{-1} = M(1/\lambda) = \bracks*{\indII{i\leq j}\frac{1}{(i-j)!}\derivLong{\frac{1}{\lambda}}{\lambda}{i-j}}_{n\times n} = \bracks*{\indII{i\leq j}\frac{(-1)^{j-i}}{\lambda^{j-i+1}}}_{n\times n} \]
                    \newline
                    (M演算を使わない方法):
                    \par
                    ...
                    \[
                        ...
                        \end{bmatrix}
                        ,\quad
                        \vv = \frac{-1}{\lambda}J(\lambda,n)^{-1}\bm{b} = \bracks*{\frac{-1}{\lambda}J(\lambda,n)^{-1}[i][n]}_{n\times 1} = \bracks*{\frac{(-1)^{(n+1)-i}}{\lambda^{(n+1)-i+1}}}_{n\times 1}
                    \]
        ...
        \subsection{Jordanブロックの指数行列}
            ...
            \begin{proof}
                ...
                よって$\bracks*{J(\lambda, n)t}^p,\quad p\in\naturalNumbers+\{0\}$の対角成分より下側の成分は$0$であることがわかる。
                \par
                \begin{equation}
                    \bracks*{J(\lambda, n)t}^p = t^p(\Gamma+U)^p = t^p\sum_{k=0}^{p}{\combi{p}{k}U^k\Gamma^{p-k}} \tag{1}
                \end{equation}
                ...
                \begin{align*}
                    ...
                    &= \sum_{l=0}^{\infty}{\frac{1}{\parens*{l+(j-i)}!}t^{l+(j-i)}\lambda^l\frac{\parens*{l+(j-i)}!}{(j-i)!l!}}\quad(l=p-(j-i)\mathrm{とおいた})\\
    ...
    \section{Perronの定理}
        \begin{shadebox}
            ...
            \end{enumerate}
        \end{shadebox}
        Perron-Frobeniusの定理の主張との違いは2.の「$\rho(A)$は他のどの固有値の絶対値よりも真に大きい」である。
        \begin{proof}
            1.と3.および2.の「$\rho(A)$は$A$の単純固有値の1つと等しく」は\cite{システム線形代数}のPerron-Frobeniusの定理の証明と同じ方法で証明できる。
            2.の「$\rho(A)$は他のどの固有値の絶対値よりも真に大きい」を，\cite{システム線形代数} 170頁 定理6.11のPerron-Frobeniusの定理の証明を利用して示す。
            その証明の中に
            \begin{equation}
                \lambda_ix_i = \sum_{j=1}^{n}{a_{ij}x_j}
            \end{equation}
            \[|\lambda||x_i| = \abs{\sum_{j=1}^{n}{a_{ij}x_j}} \leq \sum_{j=1}^{n}{a_{ij}|x_j|}\]
            という式があるが，$|\lambda|\leq\hat{\lambda}$は既に示されているので，上式に於いて等号が成り立てば$\lambda=\hat{\lambda}$であることを示せばよい。
            \par
            $A$は正行列だから$a_{ij}>0$である。$\vx$は固有ベクトルであるから$x_j$の少なくともどれか一つは非零である。
