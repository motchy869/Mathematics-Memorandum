    ...
    \section{2-ノルム(2-演算子ノルム)}
        ...
        \subsection{\texorpdfstring{$\max_{\norm{x}=1} \norm{A\vx} = \sqrt{\lambda_{\mathrm{max}}(A^\top A)}\;(A\in\realNumbers^{n\times n})$}{max\_\{||x||=1\} ||Ax|| = sqrt(λ\_max(A\string^⊤ A)) (A∈R\string^(n×n))}}
            ...
            \begin{proof}
                ...
                $\bm{b}_{i1},\bm{b}_{i2},\dots,\bm{b}_{{in}_i}$を$W\left(\lambda_i;A\right)$の正規直交基とすると，
                ...
                \[(A^\top A\vx,\vx) = \left(\sum_{i=1}^r{\lambda_i\sum_{j=1}^{n_i}{c_{ij}\bm{b}_{ij}}}\;\;\;\;,\;\;\; \sum_{i=1}^r{\sum_{j=1}^{n_i}{c_{ij}\bm{b}_{ij}}}\right) = \sum_{i=1}^r{\lambda_i\sum_{j=1}^{n_i}{{c_{ij}}^2}}\cdots(\text{ii})\]
