\chapter{行列のノルム}
    \section{2-ノルム(2-演算子ノルム)}
        ...
        \subsection{Hermite行列の絶対値最大固有値の絶対値は2-演算子ノルムと一致する}
            ...
            \end{proof}
        \subsection{\texorpdfstring{$\max_{\|x\|=1} \|A\vx\| = \sqrt{\lambda_{\mathrm{max}}(A^\top A)}\;(A\in\realNumbers^{n\times n})$}{max\_\{||x||=1\} ||Ax|| = sqrt(λ\_max(A\string^⊤ A)) (A∈R\string^(n×n))}}
            \begin{shadebox}
                実正方行列$A \in \realNumbers^{n\times n}$と実ベクトル$\vx \in \realNumbers^n$について
                \[\max_{\|x\|=1} \|A\vx\| = \sqrt{\lambda_{\mathrm{max}}(A^\top A)}\]
            \end{shadebox}
            \noindent
            (簡潔だが，高度な知識を前提とする証明)
            \begin{proof}
            ...
            (初等的な証明)
            \begin{proof}
                \[\max_{\|x\|=1} {\|A\vx\|}^2 = \lambda_{\mathrm{max}}(A^\top A)\]
                を示せば良い。\\
