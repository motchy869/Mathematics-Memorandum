    ...
    \section{応用}
        \subsection{正規化 sinc 関数の正規直交性質}
            ...
            \begin{proof}
                ...
                \end{align*}
                ここに $\psi: \realNumbers\to\complexNumbers;\;x\mapsto\mathcal{F}\parens*{t\mapsto\frac{1}{\sqrt{2\pi}}\rect{t/\pi}}(x)$ である。
                最後の行では矩形関数の Fourier 変換(次式)を用いた。
                \[ \mathcal{F}\parens{t\mapsto\rect{at}}(\omega) = \sqrt{\frac{2}{\pi}}\frac{1}{a}\sinc\frac{\omega}{a} \]
                式(1)の計算を続ける。
                \begin{align*}
                    (1) &= \sqrt{2\pi}\mathcal{F}\parens*{\parens*{t\mapsto\frac{1}{\sqrt{2\pi}}\rect{t/\pi}}^2}(n) \\
