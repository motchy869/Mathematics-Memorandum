\chapter{複素引数実数値関数}
    ...
    \section{1次増分}
        ...
        \begin{shadebox}
            $G\subseteq\complexNumbers$を開領域とし、$\RG\coloneqq\set{(x,y)\in\realNumbers^2}{x+iy\in G}$とする。
        ...
        \begin{proof}
            \quad\par
            $x=\Re{z},\;y=\Im{z},\;h=\Re{h},k=\Im{z}$とすると次式が成り立つ。
            \begin{align*}
                f(z+h) - f(z) &= \Rf(x+h,y+k) - \Rf(x,y) = \partDeriv{\Rf}{x}{}(x,y)h + \partDeriv{\Rf}{y}{}(x,y)k + o(\sqrt{h^2+k^2})
            \end{align*}
        \end{proof}
    \let\Rh\undefined
    \let\Ih\undefined
