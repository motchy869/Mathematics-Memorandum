	...
	\chapter{複素入力凸関数}
		\section{複素入力凸関数の例}
			...
			\subsection{\texorpdfstring{$\|\sum_{i=1}^N X_i\bm{a}_i + \bm{b}\|_2^2\;(\bm{a}_i, \bm{b} \in \complexNumbers^n,\;X_i \in \complexNumbers^{m\times n})$}{行列X1,X2,...,XNの線形結合×ベクトル+定数のノルム}は\texorpdfstring{$X_1,\cdots,X_N$}{X1,X2,...,XN}に関して凸である。}
				\begin{proof}
					\quad\par
					$T_X \coloneqq (X_1,\cdots,X_N),\;T_Y \coloneqq (Y_1,\cdots,Y_N),\;\lambda \in [0,1]$とし、$f(T_X) \coloneqq \|\sum_{i=1}^N X_i\bm{a}_i + \bm{b}\|_2^2$とする。
					\begin{align*}
		...
		\section{Gamma関数}
			\subsection{\texorpdfstring{$\lim_{n\to\infty} \integ{0}{n}{t^{z-1}(1+t/n)^n}{t} = \Gamma(z)\;(z\in\complexNumbers, \Re{z} > 0)$}{Gamma関数の無限乗積表示}}
				...
				\begin{proof}
					\quad\par
					$\varepsilon>0$を任意にとる。
					$G_n(z) \coloneqq \integ{0}{n}{t^{z-1}(1+t/n)^n}{t}$とする。
					\par
	...
	\chapter{Fourier級数}
		\section{連続関数のFourier級数の高周波成分は0に収束する}
			...
			\begin{proof}
				...
				も示せる。
			\end{proof}
		\section{\texorpdfstring{$\integ{-\pi}{\pi}{\frac{\sin nx}{2\tan\frac{x}{2}}}{x} = \pi\;(n\in\naturalNumbers)$}{sin(nx)/(2 tan(x/2))の[-π,π]での積分はπ}}
		...
		\section{Fourier級数の応用}
			\subsection{\texorpdfstring{$\integ{0}{\infty}{\frac{\sin x}{x}}{x} = \pi/2$}{sin(x)/xの0から∞までの積分はπ/2}}
			\begin{proof}
				\quad\par
	...
	\chapter{Fourier変換}
		...
		{%スコープ。\ge上書き回避
			...
			\begin{indentation}{1zw}{0zw}
				...
				\begin{align*}
					C(\omega) &\coloneqq \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{f(x)\cos{\omega t}}{t}\\
					S(\omega) &\coloneqq \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{f(x)\sin{\omega t}}{t}
				\end{align*}
