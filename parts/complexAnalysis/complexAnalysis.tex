\part{複素解析}
	\chapter{複素数}
		...
		\section{代数方程式}
			\subsection{諸定理}
				\subsubsection{実係数$n$次方程式が複素数解$s=\alpha+i\beta$を解に持つならばその複素共役$s^* = \alpha-i\beta$もまた解である}
					\begin{shadebox}
					...
					\end{proof}
			\subsection{例題}
				\subsection{$z^n+z^{n-1}+\dots+z^2+z+1 = 0$}
					\begin{shadebox}
						次の方程式を解け。
		...
		\section{複素入力凸関数の例}
			...
			\subsection{$\|\sum_{i=1}^N X_i\bm{a}_i + \bm{b}\|_2^2\;(\bm{a}_i, \bm{b} \in \complexNumbers^n,\;X_i \in \complexNumbers^{m\times n})$は$X_1,\cdots,X_N$に関して凸である。}
				\begin{proof}
					...
					\end{align*}
				\end{proof}
			\subsection{$\norm{A\bm{x}+\bm{b}}_2^2$が狭義凸$\iff A^*A\succ O$}
				\begin{shadebox}
	...
	\chapter{特殊関数}
		\section{Gamma関数}
			\subsection{$\lim_{n\to\infty} \integ{0}{n}{t^{z-1}(1+t/n)^n}{t} = \Gamma(z)\;(z\in\complexNumbers, \Re{z} > 0)$}
	...
	\chapter{複素積分}
		\section{例題}
			\subsection{$\integ{0}{2\pi}{\Log(1-a\exp(i\theta))}{\theta}=0\quad (C\text{: 原点中心の単位円},\;a\in\realNumbers)$}
				\begin{solution}
					\quad\par
