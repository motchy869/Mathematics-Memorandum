\part{複素解析}
	\chapter{複素数}
		...
		\section{諸公式}
			\subsection{\texorpdfstring{$x>0,\;z\in\complexNumbers$}{x>0, zは複素数}のとき\texorpdfstring{$|x^z|=x^\Re{z}$}{xのz乗の絶対値はxのRe(z)乗}}
				\begin{proof}
					\quad\par
	...
	\chapter{複素入力凸関数}
		\section{複素入力凸関数の例}
			\label{複素入力凸関数の例}
			\subsection{\texorpdfstring{$\|\sum_{i=1}^N X_i\bm{a}_i + \bm{b}\|_2^2\;(\bm{a}_i, \bm{b} \in \complexNumbers^n,\;X_i \in \complexNumbers^{m\times n})$}{行列X1,X2,...,XNの線形結合×ベクトル+定数のノルム}は\texorpdfstring{$X_1,\cdots,X_N$}{X1,X2,...,XN}に関して凸である。}
				\begin{proof}
					\quad\par
					...
					\end{align*}
				\end{proof}
			\subsection{\texorpdfstring{$\norm{A\bm{x}+\bm{b}}_2^2$}{||Ax+b||の2乗}が狭義凸\texorpdfstring{$\iff A^*A\succ O$}{⇔ A*Aが正定}}
				\begin{shadebox}
	...
	\chapter{特殊関数}
		\section{Gamma関数}
			\subsection{\texorpdfstring{$\lim_{n\to\infty} \integ{0}{n}{t^{z-1}(1+t/n)^n}{t} = \Gamma(z)\;(z\in\complexNumbers, \Re{z} > 0)$}{Gamma関数の無限乗積表示}}
				Wikipediaで紹介されている無限乗積表示の導出過程でこの主張が使われているが、その根拠が薄かったので自力で証明してみた。
				\begin{proof}
					\quad\par
					$\varepsilon>0$を任意にとる。
					$G_n(z) := \integ{0}{n}{t^{z-1}(1+t/n)^n}{t}$とする。
					\par
					Gamma関数$\integ{0}{\infty}{t^{z-1}e^{-t}}{t}$は収束するから、十分大きい$T_1(\varepsilon)>0$を取れば
					\[ \left|\integ{T_1(\varepsilon)}{\infty}{t^{z-1}e^{-t}}{t}\right| \leq \integ{T_1(\varepsilon)}{\infty}{\left|t^{z-1}e^{-t}\right|}{t} < \varepsilon/3 \]
					\par
					\ref{(1-t/n)^nは単調増加}より$(1-t/n)^n$は$n$について単調増加であり、かつ$\lim_{n\to\infty}(1-t/n)^n = e^{-t}$なので、$n \geq T_1(\varepsilon)$ならば
					\begin{align*}
						&\left|\integ{T_1(\varepsilon)}{n}{t^{z-1}(1-t/n)^n}{t}\right| \leq \integ{T_1(\varepsilon)}{n}{\left|t^{z-1}(1-t/n)^n\right|}{t} \\
						&= \integ{T_1(\varepsilon)}{n}{t^{\Re{z}-1}(1-t/n)^n}{t} \leq \integ{T_1(\varepsilon)}{\infty}{\left|t^{z-1}e^{-t}\right|}{t} < \varepsilon/3
					\end{align*}
					また、十分大きい自然数$N_1(\varepsilon,T_1(\varepsilon))$をとると、$n \geq N_1(\varepsilon,T_1(\varepsilon))$ならば次式が成り立つ。
					\[ \left|\integ{0}{T_1(\varepsilon)}{t^{z-1}\{e^{-t} - (1-t/n)^n\}}{t}\right| < \varepsilon/3 \]
					以上より、$n \geq \max\{T_1(\varepsilon), N_1(\varepsilon,T_1(\varepsilon))\}$ならば次式が成り立つ。
					\begin{align*}
						\left|\Gamma(z) - G_n(z)\right| &= \left|\integ{T_1(\varepsilon)}{\infty}{t^{z-1}e^{-t}}{t} + \integ{0}{T_1(\varepsilon)}{t^{z-1}\{e^{-t} - (1-t/n)^n\}}{t} - \integ{T_1(\varepsilon)}{n}{t^{z-1}(1-t/n)^n}{t}\right| \\
						&\leq \left|\integ{T_1(\varepsilon)}{\infty}{t^{z-1}e^{-t}}{t}\right| + \left|\integ{0}{T_1(\varepsilon)}{t^{z-1}\{e^{-t} - (1-t/n)^n\}}{t}\right| + \left|\integ{T_1(\varepsilon)}{n}{t^{z-1}(1-t/n)^n}{t}\right| < \varepsilon
					\end{align*}
				\end{proof}
	\chapter{複素積分}
		\section{例題}
			\subsection{\texorpdfstring{$\integ{0}{2\pi}{\Log(1-a\exp(i\theta))}{\theta}=0\quad (C\text{: 原点中心の単位円},\;a\in\realNumbers)$}{Log(1-a exp(iΘ))の単位円周上での積分は0}}
				\begin{solution}
					\quad\par
					$a=0$のときは明らかに成り立つ。$a\neq0$のときは$z=e^{i\theta}$とすると
					\[\integ{0}{2\pi}{\Log(1-ae^{i\theta})}{\theta} = \integ{C}{}{\frac{\Log(1-az)}{iz}}{z}\quad (C:\text{原点中心の左回り単位円})\]
	...
	\chapter{Fourier級数}
		\section{連続関数のFourier級数の高周波成分は0に収束する}
			...
			\begin{proof}
				\quad\par
				まず
				\[ \lim_{k\to\infty}\integ{-\pi}{\pi}{f(x)\sin kx}{x} = 0 \]
				を示す。ここで
				\[ \integ{-\pi}{\pi}{f(x)\sin kx}{x} = \integ{0}{\pi}{(f(x)-f(-x))\sin kx}{x} \]
				であり、$f(x)-f(-x)$は$[0,\pi]$で連続なので結局のところ区間$[0,\pi]$の任意の連続関数$f$に対して
				\[ \lim_{k\to\infty}\integ{0}{\pi}{f(x)\sin kx}{x} = 0 \]
				が成り立つことを示せば良い。
				...
				\begin{align*}
					\integ{0}{\pi}{f(x)\sin kx}{x} &= \sum_{l=0}^{\floor{k/2}-1}\left(\integ{2l\pi/k}{(2l+1)\pi/k}{f(x) \sin kx}{x} + \integ{(2l+1)\pi/k}{(2l+2)\pi/k}{f(x) \sin kx}{x}\right) \\
					&\quad + \integ{2\floor{k/2}\pi/k}{\pi}{f(x)\sin kx}{x}
				\end{align*}
				最後の項は$k\to\infty$のとき$0$に収束する。
				最初の2項は$y=kx$と変数変換して
				\begin{align*}
					&\quad \sum_{l=0}^{\floor{k/2}-1}\frac{1}{k}\left(\integ{2l\pi}{(2l+1)\pi}{f(y/k) \sin y}{y} + \integ{(2l+1)\pi}{(2l+2)\pi}{f(y/k) \sin y}{y}\right) \\
					&= \frac{1}{k} \sum_{l=0}^{\floor{k/2}-1} \integ{2l\pi}{(2l+1)\pi}{(f(y/k) - f(y/k+\pi/k)) \sin y}{y}
				\end{align*}
				となる。これの絶対値は
				\begin{align*}
					&\leq \frac{1}{k} \sum_{l=0}^{\floor{k/2}-1} \integ{2l\pi}{(2l+1)\pi}{\abs{f(y/k) - f(y/k+\pi/k) \sin y}}{y} \\
					&< \frac{1}{k} \sum_{l=0}^{\floor{k/2}-1} \integ{2l\pi}{(2l+1)\pi}{\varepsilon\abs{\sin y}}{y} = \frac{2\varepsilon}{k} \sum_{l=0}^{\floor{k/2}-1}1 = \frac{2\varepsilon}{k}\floor{k/2} \leq \frac{2\varepsilon}{k}\frac{k}{2} = \varepsilon
				\end{align*}
				同様にして
				\[ \lim_{k\to\infty}\integ{-\pi}{\pi}{f(x)\cos kx}{x} = 0 \]
				も示せる。
			\end{proof}
		\section{$\integ{-\pi}{\pi}{\frac{\sin nx}{2\tan\frac{x}{2}}}{x} = \pi\;(n\in\naturalNumbers)$}
			\label{integ{-pi}{pi}{(sin nx)/2tan(x/2)}{x} = pi}
			\begin{proof}
				\quad\par
				求めたい積分の値を$I_n$とする。
				分子の$\sin nx$について$nx = (n-1/2)x + x/2$として加法定理により展開すると
				\[ I_n = \frac{1}{2}\integ{-\pi}{\pi}{\frac{\cos^2(x/2)}{\sin(x/2)}\sin(n-1/2)x}{x} + \frac{1}{2}\integ{-\pi}{\pi}{\cos(x/2)\cos(n-1/2)x}{x} \]
				第一項の分子で$\cos^2(x/2) = 1-\sin^2(x/2)$とすると
				\begin{align*}
					I_n &= \frac{1}{2}\integ{-\pi}{\pi}{\frac{\sin(n-1/2)x}{\sin(x/2)}}{x} - \frac{1}{2}\integ{-\pi}{\pi}{\sin(x/2)\sin(n-1/2)x}{x} + \frac{1}{2}\integ{-\pi}{\pi}{\cos(x/2)\cos(n-1/2)x}{x} \\
					&= \frac{1}{2}\integ{-\pi}{\pi}{\frac{\sin(n-1/2)x}{\sin(x/2)}}{x} + \frac{1}{2}\integ{-\pi}{\pi}{\cos nx}{x} \quad (\text{↑の第2,3項を加法定理でまとめた}) \\
					&= \frac{1}{2}\integ{-\pi}{\pi}{\frac{\sin(n-1/2)x}{\sin(x/2)}}{x} \quad (\because\;\text{第2項は}0)
				\end{align*}
				さらに$(n-1/2)x = (n-1)x + x/2$として加法定理を使うと
				\[
					I_n = \frac{1}{2}\integ{-\pi}{\pi}{\frac{\sin(n-1)x\;\cos(x/2)}{\sin(x/2)}}{x} + \frac{1}{2}\integ{-\pi}{\pi}{\cos(n-1)x}{x} = I_{n-1} +
					\begin{cases}
						\pi & (n=1) \\
						0 & (\text{otherwise})
					\end{cases}
		...
		\section{Fourier級数の応用}
			\subsection{$\integ{0}{\infty}{\frac{\sin x}{x}}{x} = \pi/2$}
			\begin{proof}
				\quad\par
				...
				よって、
				\begin{align*}
					\integ{0}{\infty}{\frac{\sin x}{x}}{x} &= \lim_{n\to\infty}\integ{0}{n\pi}{\frac{\sin x}{x}}{x} = \lim_{n\to\infty}\integ{0}{\pi}{\frac{\sin nt}{t}}{t} \quad (t=x/n\text{とおいた}) \\
					&= \lim_{n\to\infty}\frac{1}{2}\integ{-\pi}{\pi}{\frac{\sin nt}{t}}{t} = \lim_{n\to\infty}\frac{1}{2}\integ{-\pi}{\pi}{\left(\frac{1}{t} - \frac{1}{2\tan(t/2)} + \frac{1}{2\tan(t/2)}\right)\sin nt}{t}
				\end{align*}
				$\frac{1}{t} - \frac{1}{2\tan(t/2)}$は$[-\pi,\pi]$で連続だから\ref{連続関数のFourier級数の高周波成分は0に収束する}より
				\[ \lim_{n\to\infty}\frac{1}{2}\integ{-\pi}{\pi}{\left(\frac{1}{t} - \frac{1}{2\tan(t/2)}\right)\sin nt}{t} = 0 \]
	...
	\chapter{Fourier変換}
		...
		{%スコープ。\ge上書き回避
			...
			\begin{indentation}{1zw}{0zw}
				...
				\begin{align*}
					C(\omega) &:= \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{f(x)\cos{\omega t}}{t}\\
					S(\omega) &:= \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{f(x)\sin{\omega t}}{t}
				\end{align*}
				$C,S$はそれぞれ\textcolor{red}{$\omega$の偶関数,奇関数}になっていることに注意。そして$t\geq0$において
				\begin{equation}
					f(t) \sim \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{C(\omega)\cos{\omega t}}{\omega} = \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{S(\omega)\sin{\omega t}}{\omega} \nonumber
				\end{equation}
				...
				\begin{proof}
					...
					\begin{align*}
						\Ge(\omega) &= \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{\ge(t)e^{-i\omega t}}{t} = \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{\ge(t)(\cos{\omega t}-i\sin{\omega t})}{t}\\
						&= \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{\ge(t)\cos{\omega t}}{t}\;\;\;(\because \ge(t)\sin{\omega t}\text{は奇関数なので対称積分の結果は0})\\
						&= \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{\ge(t)\cos{\omega t}}{t} = \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{f(t)\cos{\omega t}}{t} = C(\omega)
					\end{align*}
					となり余弦変換と一致する。同様にして
					\[\Go(\omega) = -iS(\omega)\]
					となることも確かめられる。
					...
					\begin{align*}
						\ge(t) &\sim \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{\Ge(\omega)e^{i\omega t}}{\omega} = \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{C(\omega)e^{i\omega t}}{\omega} = \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{C(\omega)(\cos{\omega t}+i\sin{\omega t})}{\omega}\\
						&= \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{C(\omega)\cos{\omega t}}{\omega}\;\;(\because C(\omega)\sin{\omega t}\text{ は奇関数なので対称積分の結果は0})\\
						&= \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{C(\omega)\cos{\omega t}}{\omega}
					\end{align*}
					\begin{align*}
						\go(t) &\sim \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{\Go(\omega)e^{i\omega t}}{\omega} = \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{-iS(\omega)e^{i\omega t}}{\omega} = \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{-iS(\omega)(\cos{\omega t}+i\sin{\omega t})}{\omega}\\
						&= \frac{1}{\sqrt{2\pi}}\integ{-\infty}{\infty}{S(\omega)\sin{\omega t}}{\omega}\;\;(\because S(\omega)\cos{\omega t}\text{ は奇関数なので対称積分の結果は0})\\
						&= \sqrt{\frac{2}{\pi}}\integ{0}{\infty}{S(\omega)\sin{\omega t}}{\omega}
					\end{align*}
	...
	\chapter{Bessel関数}
		\section{定義}
			...
			\begin{equation}
				...
				J_n(z) = \frac{1}{2\pi}\integ{-\pi}{\pi}{e^{iz\sin x}e^{-inx}}{x}
			\end{equation}
			\subsection{簡単化}
				式\eqref{eqn:explicit definition of 1st kind n-th Bessel function}は次のようにして簡単化できる。
				\[ J_n(z) = \frac{1}{2\pi}\integ{-\pi}{\pi}{\exp i\left(z\sin x -nx\right)}{x} = \frac{1}{2\pi}\integ{-\pi}{\pi}{\cos\left(z\sin x -nx\right)}{x} + \frac{1}{2\pi}\integ{-\pi}{\pi}{i\sin\left(z\sin x -nx\right)}{x} \]
				第1項は偶関数の対称積分であり、第2項は奇関数の対称積分なので0だから結局次のようになる。
				\[ J_n(z) = \frac{1}{2\pi}\integ{-\pi}{\pi}{\cos\left(z\sin x -nx\right)}{x} = \frac{1}{\pi}\integ{0}{\pi}{\cos\left(z\sin x -nx\right)}{x} \]
		\section{Maclaurin展開}
			...
			$n\geq 0$のとき、$(e^{ix}-e^{-ix})^k$の展開で$e^{inx}$が生じるのは$k=n+2l\;(l=0,1,2,\cdots)$のときで、その係数は$(-1)^l\binom{k}{l} = (-1)^l\binom{n+2l}{l}$である。
			$n<0$のとき、$(e^{ix}-e^{-ix})^k$の展開で$e^{inx}$が生じるのは$k=-n+2l\;(l=0,1,2,\cdots)$のときで、その係数は$(-1)^{k-l}\binom{k}{l} = (-1)^{-n+l}\binom{-n+2l}{l}$である。
