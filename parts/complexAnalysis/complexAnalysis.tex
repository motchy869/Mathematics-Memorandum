\part{複素解析}
	\chapter{複素数}
		\section{複素数の定義}
			任意の2つの実数$a,b\in\realSpace$に対する2項組$(a,b)$\;(※ここでは(,)という記号を用いているが、どんな記号を用いるかはどうでもいい。別の記号を使っても差し支えない。)を「\textcolor{red}{複素数}」と\underline{名付ける}。
			複素数$(a,b)$の第1要素$a$を「\textcolor{red}{実部}」、第2要素$b$を「\textcolor{red}{虚部}」と\underline{名付ける}。
			複素数全体の集合を$\complexSpace$と表記することにする。
			\par
			任意の2つの複素数$(a_1,b_1),(a_2,b_2)\in\complexSpace$に対して演算「和(+)」,「実数倍(スカラー倍)」及び「積($\times$)」を次のように\textcolor{red}{定義}する。
			\begin{align*}
		...
		\section{諸公式}
			\subsection{$x>0,\;z\in\complexSpace$のとき$|x^z|=x^\Re{z}$}
				\begin{proof}
					\quad\par
					$z = a + bi\;(a,b\in\realSpace)$とする。
	...
	\chapter{特殊関数}
		\section{Gamma関数}
			\subsection{$\lim_{n\to\infty} \integ{0}{n}{t^{z-1}(1+t/n)^n}{t} = \Gamma(z)\;(z\in\complexSpace, \Re{z} > 0)$}
	...
	\chapter{複素積分}
		\section{例題}
			\subsection{$\integ{0}{2\pi}{\Log(1-a\exp(i\theta))}{\theta}=0\quad (C\text{: 原点中心の単位円},\;a\in\realSpace)$}
				\begin{solution}
					\quad\par
	...
	\chapter{Bessel関数}
		\section{定義}
			$x\in\realSpace,\;z\in\complexSpace$に対して$e^{iz\sin x}$のFourier級数展開により「$n$次の第一種Bessel関数$J_n(z)$」を次のように定義する。
			\begin{equation}
