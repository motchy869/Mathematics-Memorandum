\chapter{固有値,固有ベクトル}
    \section{諸定理}
        ...
        \subsection{コンパニオン行列}
            ...
            \end{proof}
        \subsection{巡回行列の固有値}
            (注意) ここで「巡回行列」と呼ばれるものは\ref{巡回行列の可換則}に於けるそれとは異なる。
            \begin{shadebox}
                $N\in\naturalNumbers,\;a_0,a_1,\dots,a_{N-1}\in\complexNumbers,\;\bm{a}\coloneqq[a_0,\dots,a_{N-1}]^\top$とする。
            ...
            \begin{proof}
                ...
                \[ \bm{a}_m = \cycConv{\bm{a}}{\bm{e}_m} \]
                巡回畳み込みのDFTの性質より次式が成り立つ。
                \[ \DFT{\bm{a}_m} = \sqrt{N}\DFT{\bm{a}}\HadamardProd\DFT{\bm{e}_m} = \sqrt{N}\DFT{\bm{a}}\HadamardProd W^*[:,m] \]
                これより次式が成り立つ。
                \begin{align*}
                    W^*A &= [\DFT{\bm{a}_0},\DFT{\bm{a}_1},\dots,\DFT{\bm{a}_{N-1}}] \\
                    &= \sqrt{N}[\DFT{\bm{a}}\HadamardProd W^*[:,0],\DFT{\bm{a}}\HadamardProd W^*[:,1],\dots,\DFT{\bm{a}}\HadamardProd W^*[:,N-1]] \\
                    &= \sqrt{N}\diag{\DFT{\bm{a}}}W^*
                \end{align*}
