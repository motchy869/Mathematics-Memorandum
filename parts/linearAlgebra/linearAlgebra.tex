		...
		\section{2-ノルム(2-演算子ノルム)}
			...
			\subsection{逆行列のノルムは元の行列の最小特異値の逆数と等しい}
				...
				\begin{proof}
					$A$の特異値分解より明らか。
				\end{proof}
			\subsection{$A\in\realNumbers^{m\times n}$の各列ベクトルのノルムが高々$a$ならば$\norm{A}_2\leq a\sqrt{n}$}
				\label{th:mxn実行列Aの各列ベクトルのノルムが高々aならなAのノルムは高々a.sqrt(n)}
				\begin{proof}
					\quad\par
					$A$の第$i$列ベクトルを$\bm{a}_i$とし、$\bm{x}\in\realNumbers^n,\norm{\bm{x}}_2=1$とすると
				...
				\end{proof}
				\subsubsection{系:\quad$A\in\realNumbers^{m\times n}$の全要素の絶対値が$\varepsilon$以下であれば$\norm{A}_2\leq \varepsilon\sqrt{mn}$}
					\begin{proof}
