\part{線形代数}
	...
	\chapter{行列の演算}
		...
		\section{巡回行列の可換則}
			\begin{proof}
				...
				\end{align*}
			\end{proof}
		\section{$\realSpace^{\textcolor{red}{2}}$の対称行列同士の積は対称}
			\begin{proof}
				$
					A = \begin{bmatrix}
						a & b \\
	...
	\chapter{ベクトル空間}
		...
		\section{諸注意}
			\subsection{
				...
				\begin{proof}
					例えば$\realSpace^3$で考えて$\bm{v}_1=\bm{e}_1,\bm{v}_2=\bm{e}_2,\bm{w}_1=\bm{e}_3,\bm{w}_2=\bm{e}_1+\bm{e}_2+\bm{e}_3$
				\end{proof}
			\subsection{$W_i\cap W_j=\{\bm{0}\}\;(i\in\{1,\cdots,n\},\;i\neq j)$であっても$W_1+\cdots+W_n = W_1\oplus\cdots\oplus W_n$とは限らない}
				\begin{proof}
					\quad\par
					例えば$W_1,W_2,W_3\subseteq\realSpace^2,\;W_1=\{\bm{e}_1\},W_2=\{\bm{e}_2\},W_3=\{\bm{e}_1+\bm{e}_2\}$とするとベクトル$\bm{e}_1+\bm{e}_2$に対して分解が一意でない($W_1,W_2,W_3$から$\bm{e}_1,\bm{e}_2,\bm{0}$を持ってくる方法と、$W_1,W_2,W_3$から$\bm{0},\bm{0},\bm{e}_1+\bm{e}_2$を持ってくる方法がある)。
				\end{proof}
			\subsection{$W=V\oplus X,\;W=V\oplus Y$でも$X=Y$とは限らない}
				\begin{proof}
					例えば$W=\realSpace^2,\;V=\{[1,0]^\top\},\;X=\{[0,1]^\top\},\;Y=\{[1,1]^\top\}$
				\end{proof}
			\subsection{$\Span{\bm{a},\bm{b},\bm{c}}/\Span{\bm{a},\bm{b}} = \Span{\bm{c}}$とは限らない}
				\begin{proof}
					\quad\par
					例えば$\bm{a}=[1,0,0]^\top,\;\bm{b}=[0,1,0]^\top,\;\bm{c}=[0,0,1]^\top$、すなわち$\Span{\bm{a},\bm{b},\bm{c}}=\realSpace^3$のとき、
					$\Span{\bm{a},\bm{b},\bm{c}}/\Span{\bm{a},\bm{b}}$は$\realSpace^3$から第1要素が0である点だけを抜いたものであり、$[x,*,*]^\top,\;x\neq 0$のような点は残っている。
					このような点は$\Span{\bm{c}}$には属さない。
				\end{proof}
	\chapter{線形写像}
		\section{表記の規則}
			\subsection{一般のベクトルを要素とする配列と行列の積}
				$\bm{v}_1,\cdots,\bm{v}_n$をベクトルとする($\realSpace^n$や$\complexSpace^n$の要素とは限らないことに注意)。
		...
		\section{$\dim{V}=\dim{W}$ならば全単射線形写像$f:V\to W$が存在する}
			\begin{proof}
				...
				写像$f:V\to W$を$f(\bm{v}_i)=\bm{w}_i\;(i=1,\cdots,n),\;^\forall\bm{x},\bm{y}\in V,\alpha,\beta\in\realSpace,f(\alpha\bm{x}+\beta\bm{y})=\alpha f(\bm{x})+\beta f(\bm{y})$で定めると$f$は線形写像である。
	...
	\chapter{双対空間}
		\section{基底変換と座標変換の関係}
			\begin{shadebox}
				$V$を$n$次元ベクトル空間とし($\realSpace^n$や$\complexSpace^n$とは限らないことに注意)、$\{\bm{v}_1,\cdots,\bm{v}_n\}$と$\{{\bm{v}_1}',\cdots,{\bm{v}_n}'\}$を$V$の基底とする。
	...
	\chapter{行列式}
		...
		\section{右下要素が$0$の対称行列の行列式}
			...
			\end{proof}
			\end{comment}
			\section{$A\in\realSpace^{n\times m}, B\in\realSpace^{m\times n}$に対して$|I_m-AB| = |I_n-BA|$}
				\begin{proof}
					\[
						M := \begin{bmatrix}
							I_n & A \\
			...
			\section{系: $\left|I_n-\bm{v}\bm{v}^\top\right| = 1-\norm{\bm{v}}_2^2$}
				\begin{shadebox}
					$\bm{v} \in \complexSpace^n$に対して$\left|I_n-\bm{v}\bm{v}^\top\right| = 1-\norm{\bm{v}}_2^2$
				\end{shadebox}
				\begin{comment}
				\begin{proof}
					\begin{align*}
	...
	\chapter{逆行列}
		...
		\section{上(下)三角行列が逆行列をもてば、それも上(下)三角行列である}
			\begin{proof}
				\quad\par
				$A\in \complexSpace^{n\times n}$を正則な上三角行列とすると、$A$の対角成分は全て非零である(さもなくば$\det{A}=0$となり、正則でない)ので、
		...
		\section{1列(or 1行)だけ置き換えた行列の逆行列}
			$A=[\bm{a}_1,\cdots,\bm{a}_n]\in\complexSpace^{n\times n}$を正則な行列とし、$c$列目を置き換えた行列$A'=[\bm{a}_1,\cdots,\bm{a}'_c,\cdots,\bm{a}_n]$も正則であるとする。
	...
	\chapter{行列の階数}
		...
		\section{フルランク条件}
			\begin{shadebox}
				行列$A\in\complexSpace^{m\times n}(m\leq n)$の階数が$m$である(=$A$がフルランク)$\Leftrightarrow$($\bm{x}^\top A=\bm{0}_{1\times n}\Rightarrow\bm{x}=\bm{0}_{m\times1}$)
			\end{shadebox}
			\begin{proof}
				\quad\par
		...
		\section{優対角正方行列は正則}
			\begin{proof}
				\quad\par
				$A=[a_{ij}]\in\complexSpace^{n\times n}$を優対角行列とし、$A\bm{x}=\bm{0}_n,\;\bm{x}=[x_1,\cdots,x_n]^\top\in\complexSpace^n$とおく。
				$A\bm{x}$の第1成分の絶対値について
				\[ |a_{11}x_1 + \cdots a_{1n}x_n| \geq |a_{11}||x_1| - \cdots - |a_{1n}||x_n| \]
	...
	\chapter{固有値,固有ベクトル}
		\section{諸定理}
			\subsection{相異なる固有値に対応する固有ベクトルは一次独立}
				...
				\begin{shadebox}
					$A\in\complexSpace^{n\times n}$の相異なる$p$個の固有値$\lambda_i\;(i=1,\cdots,p)$に対応する固有ベクトルを$\bm{v}_i$とすると$\bm{v}_1,\cdots,\bm{v}_p$は一次独立である。
				\end{shadebox}
				\begin{proof}
					\quad\par
		...
		\section{スペクトル写像定理}
			...
			\begin{proof}
				\quad\par
				$A$は正方行列であるから適当なユニタリ行列$P\in\complexSpace^{n\times n}$により次のように上三角化できる。
				\[A = PUP^\top\]
		...
		\section{一般固有空間}
			...
			%	\begin{shadebox}
			%		$\lambda_i \neq \lambda_j$を$A\in\complexSpace^{n\times n}$の異なる固有値とし、その代数的重複度を$n_i,n_j$とする。
			...
			\subsection{代数的重複度$\geq$幾何学的重複度}
				...
				\end{proof}
			\subsection{$AB=O$でも$\realSpace^{n\times n} = \Ker{A} + \Ker{B}$とは限らない}
				\begin{proof}
					例えば
					$
						A=B=
				...
				\begin{shadebox}
					...
					\begin{proof}
						...
						\[
							...
							\end{bmatrix}
							,\quad
							\bm{b} = [0\cdots0\;1]^\top \in \complexSpace^n
						\]
	...
	\chapter{定値性}
		\section{正定対称行列の逆行列も正定}
			\begin{proof}
				\quad\par
				$A\in\realSpace^{n\times n}$を正定対称行列とし、重複を含めた$n$個の固有値を$\lambda_1,\cdots,\lambda_n > 0$とする。
				$A$は実対称だから適当な直交行列$P\in\realSpace^{n\times n}$が存在して$A = P\diag{\lambda_1,\cdots,\lambda_n}P^\top$と表せるので
		...
		\section{$A$が半正定で$\bm{x}^\top A\bm{x}=0$ならば$\bm{x}\in\Ker{A}$}
			\begin{proof}
				...
				$\bm{x}$を$\realSpace^n$の正規直交基底$\{\bm{p}_1,\cdots,\bm{p}_n\}$で展開して$\bm{x} = \sum_{i=1}^n c_i\bm{p}_i$とすると$0 = \bm{x}^\top A\bm{x} = \sum_{i=1}^n \lambda_i {c_i}^2$だから正の固有値に対応する$c_i$は全て0である。
	...
	\chapter{ノルム}
		\section{諸定理}
			\subsection{$\norm{\bm{x}}_{p+a} \leq \norm{\bm{x}}_p\;(\bm{x}\in\realSpace^n,\;p\geq 1,\;a\geq 0)$}
				\begin{proof}
					\quad\par
					...
					\end{align*}
				\end{proof}
			\subsection{$1\leq p\leq q,\;\bm{x}\in\complexSpace^n$のとき$\norm{\bm{x}}_p \leq n^{1/p-1/q}\norm{\bm{x}}_q$}
				\begin{proof}
					\quad\par
					$\bm{a},\bm{b}\in\complexSpace^n$とする。$r=q/p, s=q/(q-p)$とすると$1\leq r,s$および$1/r+1/s=1$を満たすのでヘルダーの不等式より次式が成り立つ。
	...
	\chapter{行列のノルム}
		\section{2-ノルム(2-演算子ノルム)}
			\subsection{定義}
				\begin{shadebox}
					$A\in\complexSpace^{m\times n}$の2-ノルム(「2-演算子ノルム」とも呼ばれる)$\norm{A}_2$は次の3つの等価な式で定義される。\\
					\begin{align*}
						\norm{A}_2 &= \max\left\{\frac{\norm{A\vx}_2}{\norm{\vx}_2}:\vx\neq\bm{0}\right\}\\
						&= \max\{\norm{A\vx}_2:\norm{\vx}_2=1\}\\
						&= \min\{c>0:\norm{A\vx}_2\leq c\norm{\vx}_2,\vx\in\complexSpace^n\}
					\end{align*}
				\end{shadebox}
				\begin{proof}
					\quad\par
			...
			\subsection{スペクトル半径の上界}
				\begin{shadebox}
					$A\in\complexSpace^{n\times n}$について$\rho(A)\leq\norm{A}$
				\end{shadebox}
				\begin{proof}
					\quad\par
			...
			\subsection{実対称行列の最大固有値の絶対値はノルムと一致する}
				\begin{shadebox}
					$A\in\realSpace^{n\times n}$が対称行列であるとき$|\lambda_\mathrm{max}| = \norm{A}$
				\end{shadebox}
				\begin{proof}
					\quad\par
			...
			\subsection{$\max_{\|x\|=1} \|A\vx\| = \sqrt{\lambda_{\text{max}}\{A^\top A\}}$}
				\begin{shadebox}
					実正方行列$A \in \realSpace^{n\times n}$と実ベクトル$\vx \in \realSpace^n$について
				...
				\begin{proof}
					...
					ノルムが1であるような任意のベクトル$\vx \in \realSpace^n$は適当な実数$c_{ij}$を用いて
			...
			\subsection{逆行列のノルムは元の行列の最小特異値の逆数と等しい}
				\begin{shadebox}
					正則行列$A\in\complexSpace^{n\times n}$の特異値を$\sigma_1\geq\cdots\geq\sigma_n>0$とすると$\norm{A^{-1}}_2=1/\sigma_n$
				\end{shadebox}
				\begin{proof}
					$A$の特異値分解より明らか。
				\end{proof}
			\subsection{$A\in\realSpace^{m\times n}$の各列ベクトルのノルムが高々$a$ならば$\norm{A}_2\leq a\sqrt{n}$}
				\label{th:mxn実行列Aの各列ベクトルのノルムが高々aならなAのノルムは高々a.sqrt(n)}
				\begin{proof}
					\quad\par
					$A$の第$i$列ベクトルを$\bm{a}_i$とし、$\bm{x}\in\realSpace^n,\norm{\bm{x}}_2=1$とすると
				...
				\end{proof}
				\subsubsection{系:\quad$A\in\realSpace^{m\times n}$の全要素の絶対値が$\varepsilon$以下であれば$\norm{A}_2\leq \varepsilon\sqrt{mn}$}
					\begin{proof}
	...
	\chapter{既約性}
		\section{非負行列が既約であるための必要十分条件}
			\begin{shadebox}
				$A\in\realSpace^{n\times n},\;A\geq O$が既約であるための必要十分条件は$\left(I+A\right)^{n-1}>O$
			\end{shadebox}
			\begin{proof}
				\quad\\
				(必要)
	...
	\chapter{漏れ確率行列}
		\section{定義}
			\begin{shadebox}
				正方行列$A\in\realSpace^{n\times n}$が次の条件を満たすとき、$A$を$o(r)$の漏れ確率行列と呼ぶことにする。
				\begin{itemize}
					\item 各要素が0以上1未満
