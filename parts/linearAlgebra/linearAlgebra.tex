\part{線形代数}
	...
	\chapter{ベクトル空間}
		...
		\section{諸定理}
			...
			\subsection{一次独立なベクトル\texorpdfstring{$\bm{v}_1,\cdots,\bm{v}_k\in V$}{v1,...,vk ∈ V}に\texorpdfstring{$\bm{v}\in V$}{v∈V}を加えたものが一次独立 \texorpdfstring{$\iff\;\bm{v}$}{⇔ v}が\texorpdfstring{$\bm{v}_1,\cdots,\bm{v}_k$}{v1,...,vk}の一次結合で表せない}
				...
				\end{proof}
			\subsection{\texorpdfstring{$A_v\coloneqq\{\bm{v}_1,\cdots,\bm{v}_k\}$}{Av := \{v1,...,vk\}}と\texorpdfstring{$A_w\coloneqq\{\bm{w}_1,\cdots,\bm{w}_l\}$}{Aw := \{w1,...,wl\}}が各々一次独立で\texorpdfstring{$k>l\; \Rightarrow\; ^\exists\bm{v}\in A_v \st \bm{w}_1,\cdots,\bm{w}_l,\bm{v}$}{k>k ⇒ ∃v∈Av s.t. w1,...,wl,v}が一次独立}
				\begin{shadebox}
					線形空間$V$のベクトルの集合$A_v\coloneqq\{\bm{v}_1,\cdots,\bm{v}_k\}$と$A_w\coloneqq\{\bm{w}_1,\cdots,\bm{w}_l\}$がそれぞれ一次独立で$k>l$であれば、ある$\bm{v}\in A_v$が存在して$\bm{w}_1,\cdots,\bm{w}_l,\bm{v}$が一次独立である。
				\end{shadebox}
				\begin{proof}
					\quad\par
	...
	\chapter{線形写像}
		\section{表記の規則}
			\subsection{一般のベクトルを要素とする配列と行列の積}
				...
				\[
					...
					\end{bmatrix},
					\qquad
					[\bm{v}_1,\cdots,\bm{v}_n]B \coloneqq
		...
		\section{不変部分空間と表現行列}
			\label{不変部分空間と表現行列}
			\begin{shadebox}
				$A\in\field^{n\times n},\quad W_1,\cdots,W_m\in\field^n : A$の不変部分空間,\quad$\field^n = W_1\oplus\cdots\oplus W_m,\quad d_i \coloneqq \dim{W_i}$\\
				$\{\bm{w}_{i1},\cdots,\bm{w}_{id_i}\} : W_i$の基底,\quad$\Phi_i \coloneqq [\bm{w}_{i1},\cdots,\bm{w}_{id_i}],\quad \Phi \coloneqq [\Phi_1,\cdots,\Phi_m]$\\
	...
	\chapter{双対空間}
		\section{基底変換と座標変換の関係}
			\begin{shadebox}
				...
				$\underline{\bm{x}} \coloneqq [x_1,\cdots,x_n]^\top,\;\underline{\bm{x}}' \coloneqq [{x_1}',\cdots,{x_n}']^\top$とする。
	...
	\chapter{逆行列}
		\section{\texorpdfstring{$[\bm{\phi}_1,\cdots,\bm{\phi}_n]^{-1}\bm{\phi}_i=\bm{e}_i$}{[φ1,...,φn]\string^(-1)φi = ei}}
			...
			\begin{proof}
				\quad\par
				$\Phi \coloneqq [\bm{\phi}_1,\cdots,\bm{\phi}_n]$とする。次式より直ちに定理が証明される。
	...
	\chapter{行列の階数}
		...
		\section{\texorpdfstring{$A\in\field^{m\times n}$}{A∈F\string^(m×n)}の階数が\texorpdfstring{$n$}{n}ならば\texorpdfstring{$A^* A$}{A\string^*A}は正則}
			...
			\begin{proof}(ややこしい証明)
				...
				よって
				\begin{align*}
					T &\coloneqq [\bm{t}_1,\bm{t}_2,\cdots,\bm{t}_n]\\
					P &\coloneqq [\bm{p}_1,\bm{p}_2,\cdots,\bm{p}_n]
				\end{align*}
				とすれば
				\[AT = P\quad(A = PT^{-1})\]
		...
		\section{\texorpdfstring{$A\in\field^{m\times n}$}{A∈F\string^(m×n)}の階数が\texorpdfstring{$r$}{r}であるなら、ある正則行列\texorpdfstring{$T_x\in\field^{n\times n},\quad T_y\in\field^{m\times m}$}{Tx∈\string^(n×n), Ty∈\string^(m×m)}が存在して...}
			...
			\begin{proof}
				...
				である。$\bm{y}_i\coloneqq A\bm{x}_i\quad(i=1,2,\cdots,r)$とし、これらを含む$\field^m$の基底を$\{\bm{y}_1,\bm{y}_2,\cdots,\bm{y}_r,\bm{y}_{r+1},\cdots,\bm{y}_m\}$とすると
				$T_x\coloneqq[\bm{x}_1,\bm{x}_2,\cdots,\bm{x}_n],\quad T_y\coloneqq[\bm{y}_1,\bm{y}_2,\cdots,\bm{y}_m]$は正則であり、
				\begin{align*}
	...
	\chapter{固有値,固有ベクトル}
		\section{諸定理}
			...
			\subsection{Hadamard行列の固有値}
				...
				\begin{proof}
					\quad\par
					$2^n$次のHadamard行列を$H_n$と書く。$H_0 \coloneqq [1]$と定義する。
		...
		\section{一般固有空間}
			...
			%		一般固有空間を$F_i\coloneqq\Ker{(A-\lambda_i)^{n_i}},\;F_j\coloneqq\Ker{(A-\lambda_j)^{n_j}}$と表すと、$F_i\cap F_j=\{\bm{0}_n\}$
			%	\end{shadebox}
			\subsection{一般固有空間への直和分解}
				...
				\begin{proof}
					...
					$F_A(\lambda_i)$の基底を$\{\bm{\phi}_{i1},\cdots,\bm{\phi}_{i{\nu_i}'}\}$とし、$\Phi_i \coloneqq [\bm{\phi}_{i1},\cdots,\bm{\phi}_{i{\nu_i}'}],\quad \Phi \coloneqq [\Phi_i,\cdots,\Phi_p]\in\field^{n\times n}$とすれば、
			...
			\subsection{Jordanブロックの指数行列}
				...
				\begin{proof}
					...
					\[
						...
						\Gamma\coloneqq\lambda I_{n\times n},\quad
						U\coloneqq
						\begin{bmatrix}
							0		& 1	& 0			& \cdots	& 0\\
									& 0	& 1			& \ddots	& \vdots\\
		...
		\section{2-ノルム(2-演算子ノルム)}
			\subsection{定義}
				...
				\begin{proof}
					...
					\[S_1 \coloneqq \left\{\frac{\norm{A\vx}_2}{\norm{\vx}_2}:\vx\neq\bm{0}\right\},\quad S_2 \coloneqq \{\norm{A\vx}_2:\norm{\vx}_2=1\},\quad S_3 \coloneqq \{c>0:\norm{A\vx}_2\leq c\norm{\vx}_2,\vx\in\complexNumbers^n\}\]
					まず
					\[S_2 = \left\{\frac{\norm{A\vx}_2}{\norm{\vx}_2}:\norm{\vx}_2=1\right\}\subseteq S_1\]
					より$c_1\geq c_2$である。
					\par$c_1$を与えるベクトル$\vx_1$に対して$\vx_1' \coloneqq \vx_1/\norm{\vx_1}_2$とすれば$\norm{\vx_1'}_2=1$であり、$\norm{A\vx_1'}_2=\frac{\norm{A\vx_1}_2}{\norm{\vx_1}_2}$であるからつまり$c_1\in S_2$である($S_1$の最大要素が$S_2$に属する)。
			...
			\subsection{スペクトル半径の上界}
				...
				\begin{proof}
					...
					\[\rho(A) \coloneqq |\lambda_\mathrm{max}| = \frac{\norm{A\vx}}{\norm{\vx}} \leq \max_{\vy\neq\bm{0}}\frac{\norm{A\vy}}{\norm{\vy}} =: \norm{A}\]
				\end{proof}
			\subsection{Hermite行列の絶対値最大固有値の絶対値は2-演算子ノルムと一致する}
				...
				\begin{proof}
					...
					よって
					\[\norm{A\vx}^2 = \inprod{A\vx}{A\vx} = \sum_{i=1}^{\sigma}{{\lambda_i}^2\sum_{k=1}^{n_i}{|c_{k,i}|^2}} = \sum_{i=1}^{\sigma}{c_i{\lambda_i}^2}\quad\left(c_i \coloneqq \sum_{k=1}^{n_i}{|c_{k,i}|^2},\quad\sum_{i=1}^{\sigma}{c_i}=1\right)\]
			...
			\subsection{\texorpdfstring{$\max_{\|x\|=1} \|A\vx\| = \sqrt{\lambda_{\text{max}}(A^\top A)}\;(A\in\realNumbers^{n\times n})$}{max\_\{||x||=1\} ||Ax|| = sqrt(λ\_max(A\string^⊤ A)) (A∈R\string^(n×n))}}
				...
				\begin{proof}
					...
					$\lambda_1 \geq \lambda_2 \geq \dots \geq \lambda_r$を$A^\top A$の固有値とし、$n_i \coloneqq \text{dim}\left(W\left(\lambda_i;A\right)\right)$とする。\\
	...
	\chapter{凸領域}
		\section{凸領域内のベクトルの凸結合は元の凸領域に属す}
			...
			\begin{proof}
				...
				$n=m+1$のとき、
				\[ \sum_{i=1}^n\alpha_i\bm{v}_i = \sum_{i=1}^{m+1}\alpha_i\bm{v}_i = \sum_{i=1}^m\alpha_i\bm{v}_i + \alpha_{m+1}\bm{v}_{m+1} = A_m\sum_{i=1}^m\beta_i\bm{v}_i + \alpha_{m+1}\bm{v}_{m+1} \quad \left(A_m \coloneqq \sum_{i=1}^m\alpha_i,\;\beta_i \coloneqq \frac{\alpha_i}{A_m}\right) \]
				$\sum_{i=1}^m\beta_i = 1$であるから、仮定より$\bm{w}_{m+1} \coloneqq \sum_{i=1}^m\beta_i\bm{v}_i\in V$である。
