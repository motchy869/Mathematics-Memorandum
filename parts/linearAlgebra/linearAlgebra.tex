	...
	\chapter{行列のノルム}
		\section{2-ノルム(2-演算子ノルム)}
			...
			\subsection{スペクトル半径の上界}
				...
				\end{proof}
			\subsection{実対称行列の最大固有値の絶対値はノルムと一致する}
				\begin{shadebox}
					$A\in\realNumbers^{n\times n}$が対称行列であるとき$|\lambda_\mathrm{max}| = \norm{A}$
				\end{shadebox}
				\begin{proof}
					\quad\par
					$A$は実対称行列であるから固有値は全て実数で、$A$は実直交行列で対角化できることは既知とする。
					$A$の異なる固有値を大きい順に$\lambda_1,\lambda_2,\cdots,\lambda_\sigma\quad(\sigma\leq n)$とし、$\lambda_i$に属する固有空間$W(\lambda_i;A)$の次元を$n_i$とする。
					$\sum_{i=1}^{\sigma}{n_i} = n$であり、$W(\lambda_i;A)$の正規直交基底を$\{\vx_{1,i},\vx_{2,i},\cdots,\vx_{n_i,i}\}$とすると
					$\bigcup_{i=1}^{\sigma}\{\vx_{1,i},\vx_{2,i},\cdots,\vx_{n_i,i}\}$は$\realNumbers^n$の正規直交基底である。
					\par
					$\norm{A}$の定義は$\norm{A} := \max\{\norm{A\vx}:\norm{\vx}=1\}$であった。
					任意の$\vx\in\realNumbers^n,\quad\norm{\vx}=1$は適当な実数$\bigcup_{i=1}^{\sigma}\{c_{1,i},c_{2,i},\cdots,c_{n_i,i}\},\quad\sum_{i=1}^{\sigma}{\sum_{k=1}^{n_i}{{c_{k,i}}^2}}=1$を用いて
					...
					\[\norm{A} = \max\{\norm{A\vx}:\norm{\vx}=1\} = |\lambda_1| = |\lambda_\mathrm{max}|\]
				\end{proof}
