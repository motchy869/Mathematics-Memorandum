\part{線形代数}
	...
	\chapter{ベクトル空間}
		...
		\section{諸注意}
			\subsection{
				\texorpdfstring{$\{\bm{v}_1,\bm{v}_2\},\{\bm{w}_1,\bm{w}_2\},\{\bm{v}_1,\bm{v}_2,\bm{w}_1\},\{\bm{v}_1,\bm{v}_2,\bm{w}_2\}$}{\{v1,v2\},\{w1,w2\},\{v1,v2,w1\},\{v1,v2,w2\}}が各々一次独立でも\texorpdfstring{$\{\bm{v}_1,\bm{v}_2,\bm{w}_1,\bm{w}_2\}$}{\{v1,v2,w1,w2\}}が一次独立とは限らない}
				\begin{proof}
					例えば$\realNumbers^4$で考えて$\bm{v}_1=\bm{e}_1,\bm{v}_2=\bm{e}_2,\bm{w}_1=\bm{e}_3,\bm{w}_2=\bm{e}_1+\bm{e}_2+\bm{e}_3$
				\end{proof}
			\subsection{\texorpdfstring{$W_i\cap W_j=\{\bm{0}\}\;(i\in\{1,\cdots,n\},\;i\neq j)$}{Wi⋂Wj=\{0\}}であっても\texorpdfstring{$W_1+\cdots+W_n = W_1\oplus\cdots\oplus W_n$}{W1+...+Wn = W1⊕...⊕Wn}とは限らない}
				\begin{proof}
					\quad\par
	...
	\chapter{行列式}
		...
		\section{逆対角転置行列の行列式は元の行列のそれと等しい}
			...
			\end{proof}
	\chapter{逆行列}
		\section{\texorpdfstring{$[\bm{\phi}_1,\cdots,\bm{\phi}_n]^{-1}\bm{\phi}_i=\bm{e}_i$}{[φ1,...,φn]\string^(-1)φi = ei}}
	...
	\chapter{行列の階数}
		...
		\section{フルランク条件}
			...
			\end{proof}
		\section{\texorpdfstring{$A \in \complexNumbers^{m\times n},\;\rank{A} = \rank{A^* A} = \rank{AA^*}$}{A∈C\string^(m×n), rank(A) = rank(A\string^*A) = rank(AA\string^*)}}
			\begin{proof}
				\quad\par
			...
			\end{proof}
		\section{\texorpdfstring{$A\in\field^{m\times n}$}{A∈F\string^(m×n)}の階数が\texorpdfstring{$n$}{n}ならば\texorpdfstring{$A^* A$}{A\string^*A}は正則}
	...
	\chapter{定値性}
		...
		\section{非対称行列は固有値が全て正でも正定とは限らない}
			\begin{proof}
				...
				$
			\end{proof}
		\section{\texorpdfstring{$A$}{A}: Hermite対称で正定\texorpdfstring{$\Rightarrow (\bm{x}^*A\bm{x})(\bm{y}^*A^{-1}\bm{y}) \geq |\bm{x}^*\bm{y}|^2$}{⇒ (x\string^*Ax)(y\string^*A\string^(-1)y) ≧ |x\string^*y|\string^2}}
			\begin{shadebox}
