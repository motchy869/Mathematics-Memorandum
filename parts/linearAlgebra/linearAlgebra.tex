\part{線形代数}
	...
	\chapter{逆行列}
		...
		\section{列(or 行)を入れ替えた行列の逆行列}
			...
			\end{proof}
		\section{Sherman-Morrisonの公式の特別な場合\texorpdfstring{: $(I+\bm{u}\bm{v}^*)^{-1} = (I-\bm{u}\bm{v}^*/(1+\bm{v}^*\bm{u}))$}{}}
			\label{Sherman-Morrisonの公式の特別な場合}
			\begin{shadebox}
				$\bm{u},\bm{v} \in \complexNumbers^m,\; 1+\bm{v}^*A^{-1}\bm{u} \neq 0$とするとき次式が成り立つ。
				\[ (I+\bm{u}\bm{v}^*)^{-1} = I - \frac{1}{1+\bm{v}^*\bm{u}}\bm{u}\bm{v}^* \]
			\end{shadebox}
			\begin{proof}
				\quad\par
				Woodburyの公式の特別な場合として片付けるのが一般的だが、ここではJordan分解を用いて導出してみる。
				$\bm{u}\bm{v}^* = 0$のときは定理の主張は明らかに成り立つ。
				以下では$\bm{u}\bm{v}^* \neq 0$とする。
				仮定$1+\bm{v}^*A^{-1}\bm{u} \neq 0$と\ref{I-ABの行列式}より$I+\bm{u}\bm{v}^*$には逆行列が存在する。
				$\bm{u}\bm{v}^*$の階数は1であり($\Img{\bm{u}\bm{v}^*} = \Span{\bm{u}}$)、唯一の固有値は$\bm{v}^*\bm{u}$である(対応する固有ベクトルは$\bm{u}$)。
				よって$\bm{u}\bm{v}^*$をJordan分解すると、適当な正則行列$J$を用いて次のように表せる。
				\[
					\bm{u}\bm{v}^* =
					\begin{bmatrix}
						\bm{v}^*\bm{u} & \bm{0}^\top \\
				...
				\end{align*}
			\end{proof}
		\section{Sherman-Morrisonの公式\texorpdfstring{: $(A+\bm{u}\bm{v}^*)^{-1} = (I-A^{-1}\bm{u}\bm{v}^*/(1+\bm{v}^*A^{-1}\bm{u}))A^{-1}$}{}}
			\begin{shadebox}
				$A \in \complexNumbers^{m\times m}, \bm{u},\bm{v} \in \complexNumbers^m$とする。
				$A$は可逆であるとし、$1+\bm{v}^*A^{-1}\bm{u} \neq 0$とする。
				このとき次式が成り立つ。
				\[ (A+\bm{u}\bm{v}^*)^{-1} = \left( I - \frac{1}{1+\bm{v}^*A^{-1}\bm{u}}A^{-1}\bm{u}\bm{v}^* \right)A^{-1} \]
			\end{shadebox}
			\begin{proof}
				\quad\par
				$A+\bm{u}\bm{v}^* = A(I + A^{-1}\bm{u}\bm{v}^*)$だから$(A+\bm{u}\bm{v}^*)^{-1} = (I + A^{-1}\bm{u}\bm{v}^*)^{-1}A^{-1}$であり、2つ目の因子に\ref{Sherman-Morrisonの公式の特別な場合}を適用すればよい。
			\end{proof}
	\chapter{特性多項式}
	...
	\chapter{行列の階数}
		...
		\section{\texorpdfstring{$A \in \complexNumbers^{m\times n},\;\rank{A} = \rank{A^* A} = \rank{AA^*}$}{A∈C\string^(m×n), rank(A) = rank(A\string^*A) = rank(AA\string^*)}}
			...
			\begin{proof}
				\quad\par
				$A$の特異値分解を$U\Sigma V^*$とすると、$\rank{AA^*} = \rank{U\Sigma V^*V\Sigma U^*} = \rank{U\Sigma^2 U^*} = \rank{\Sigma^2} = \rank{\Sigma} = \rank{A}$。
				同様にして$\rank{A^*A} = \rank{A}$も示せる。
			\end{proof}
