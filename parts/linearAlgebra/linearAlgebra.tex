\part{線形代数}
	...
	\chapter{行列の演算}
		...
		\section{$\realNumbers^{\textcolor{red}{2}}$の対称行列同士の積は対称}
			...
			\end{attention}
		\section{Kronecker積}
			\subsection{混合積 $(A\otimes B)(C\otimes D) = (AC)\otimes(BD)$}
				\begin{shadebox}
	...
	\chapter{行列の階数}
		...
		\section{$A \in \complexNumbers^{m\times n},\;\rank{A} = \rank{A^* A} = \rank{AA^*}$}
			...
			\end{proof}
		\section{$A\in\field^{m\times n}$の階数が$n$ならば$A^* A$は正則}
			\label{列フルランク行列を掛けあわせて正則行列を作る}
			\begin{shadebox}
				$A\in\field^{m\times n}$が列フルランクならば$A^* A$は正則
			\end{shadebox}
			\begin{proof}(ややこしい証明)
				\par
				...
				\[\rank{A^* A} = \rank{\left(T^{-1}\right)^* P^* PT^{-1}} = \rank{\left(T^{-1}\right)^* I_{n\times n}T^{-1}} = \rank{I_{n\times n}} = n\]
			\end{proof}
			\begin{proof}
				(より簡単な証明)
				\par
				$\bm{x}\in\field^n$を任意にとって$A^* A\bm{x}=\bm{0}$とおいてみると
				\[A^* A\bm{x}=\bm{0} \Rightarrow \bm{x}^* A^* A\bm{x}=\bm{0} \iff \inprod{A\bm{x}}{A\bm{x}}=\bm{0} \iff \norm{A\bm{x}}=0 \iff A\bm{x}=\bm{0}\]
				$A$は列フルランクであったから$\bm{x}=\bm{0}$となるので$A^* A$は正則である。
			\end{proof}
	...
	\chapter{トレース}
		\section{$\tr{AB}=\tr{BA}$}
			...
			\end{proof}
		\section{Hermite行列$A,B$に対して$\tr{AB} \in \realNumbers$}
			\begin{shadebox}
	...
	\chapter{定値性}
		\section{正定Hermite対称行列の逆行列も正定}
			\begin{proof}
				\quad\par
				$A\in\complexNumbers^{n\times n}$をHermite対称な正定行列とし、重複を含めた$n$個の固有値を$\lambda_1,\cdots,\lambda_n > 0$とする。
				$A$はHermite対称だから適当なユニタリ行列$P\in\complexNumbers^{n\times n}$が存在して$A = P\diag{\lambda_1,\cdots,\lambda_n}P^*$と表せるので
				$A^{-1} = P\diag{\lambda_1,\cdots,\lambda_n}^{-1}P^* = P\diag{1/\lambda_1,\cdots,1/\lambda_n}P^*$である。
		...
		\section{非対称行列は固有値が全て正でも正定とは限らない}
			\begin{proof}
				...
				$
			\end{proof}
		\section{$A$: Hermite対称で正定$\Rightarrow (\bm{x}^*A\bm{x})(\bm{y}^*A^{-1}\bm{y}) \geq \|\bm{x}^*\bm{y}\|_2^2$}
			\begin{shadebox}
				$A \in \complexNumbers^{n\times n}$はHermite対称で正定であるとする。
				このとき、任意の$\bm{x}, \bm{y} \in \complexNumbers^n$に対して次式が成り立つ。
				\[ (\bm{x}^*A\bm{x})(\bm{y}^*A^{-1}\bm{y}) \geq \|\bm{x}^*\bm{y}\|_2^2 \]
			...
			\begin{proof}
				...
				これを用いると次式を得る。
				\[ (\bm{x}^*A\bm{x})(\bm{y}^*A^{-1}\bm{y}) = \norm{\sqrt{A}\bm{x}}_2^2 \norm{\sqrt{A^{-1}}\bm{y}}_2^2 \geq \left|\inprod{\sqrt{A}\bm{x}}{\sqrt{A^{-1}}\bm{y}}\right|^2 = |\bm{x}^*\bm{y}|^2 \]
	...
	\chapter{擬逆行列}
		...
		\section{行フルランクな行列の擬逆行列を用いた線形方程式の解がノルム最小であること}
			...
			\begin{proof}
				\quad\par
				$A^\dagger\bm{b}$が解であることと、$A^\dagger = A^*(AA^*)^{-1}$であることは既知とする(特異値分解を用いて確かめられる)。
				...
				\begin{align*}
					\norm{\bm{x}}_2^2 &= \norm{A^\dagger\bm{b}}_2^2 + \norm{\bm{v}}_2^2 + 2\Re{\inprod{\bm{v}}{A^\dagger\bm{b}}} = \norm{A^\dagger\bm{b}}_2^2 + \norm{\bm{v}}_2^2 + 2\Re{\bm{v}^*A^*(AA^*)^{-1}\bm{b}} \\
					&= \norm{A^\dagger\bm{b}}_2^2 + \norm{\bm{v}}_2^2 + 2\Re{(A\bm{v})^*(AA^*)^{-1}\bm{b}} \quad (\because \bm{v} \in \Ker{A}) \\
					&= \norm{A^\dagger\bm{b}}_2^2 + \norm{\bm{v}}_2^2 \geq \norm{A^\dagger\bm{b}}_2^2
				\end{align*}
			\end{proof}
	\chapter{ベクトルのノルム}
		...
		\section{$\norm{A\bm{x} + \bm{b}}_2^2$の最小化条件}
			\subsection{制約なしの場合}
				\begin{shadebox}
					...
					\[ A^*A\mathring{\bm{x}} = -A^*\bm{b} \]
					特に$A$が列フルランクであれば$A^*A$は正則(\ref{列フルランク行列を掛けあわせて正則行列を作る})なので$\mathring{\bm{x}}$は次式で一意に定まる。
					\[ \mathring{\bm{x}} = -(A^*A)^{-1}A^*\bm{b} \]
				\end{shadebox}
				\begin{proof}
					\quad\par
					\ref{複素入力凸関数の例}より$f$は凸であるので、$\mathring{\bm{x}}$が$f$を最小化する必要十分条件は次式である。
					...
					\begin{align*}
						g(\mathrm{d}\bm{x}) &= (A\mathrm{d}\bm{x})^*(A\mathring{\bm{x}} + \bm{b}) + (A\mathring{\bm{x}} + \bm{b})^*(A\mathrm{d}\bm{x}) = 2\Re{(\mathring{\bm{x}}^*A^*A + \bm{b}^*A)\mathrm{d}\bm{x}}
					\end{align*}
					であるから、$\mathring{\bm{x}}$に関する条件は$\mathring{\bm{x}}^*A^*A + \bm{b}^*A = O$、すなわち$A^*A\mathring{\bm{x}} = -A^*\bm{b}$である。
				\end{proof}
			\subsection{線形制約付きの場合}
				\begin{shadebox}
					$f$の定義は前小節のものとする。
					$m\in\naturalNumbers,\;\bm{c}_i \in \complexNumbers^n, d_i\in\complexNumbers,\;g_i(\bm{x}) := \bm{c}_i^\top\bm{x}+d_i$とする。
					ある$\mathring{\bm{x}} \in \complexNumbers^n, \mathring{\bm{\lambda}} \in\complexNumbers^m$に対して$g_i(\mathring{\bm{x}}) = 0\;(i=1,\cdots,m),\; (A\mathring{\bm{x}}+\bm{b})^*A = \sum_{i=1}^m \mathring{\lambda}_i\bm{c}_i^\top$であるとき、$f$は制約条件$g_i(\bm{x})=0\;(i=1,\cdots,m)$の下で$\mathring{\bm{x}}$に於いて最小値をとる。
				\end{shadebox}
				\begin{proof}
					\quad\par
					$\bm{h} \in \complexNumbers^n,\;g_i(\mathring{\bm{x}} + \bm{h}) = 0\;(i=1,\cdots,m)$とする。
					$g_i(\mathring{\bm{x}} + \bm{h}) = 0 \iff \bm{c}_i^\top\bm{h} = 0$に注意する。
					\begin{align*}
						f(\mathring{\bm{x}} + \bm{h}) &= f(\mathring{\bm{x}}) + 2\Re{(A\mathring{\bm{x}}+\bm{b})^*A\bm{h}} + \bm{h}^*A^*A\bm{h} = f(\mathring{\bm{x}}) + 2\Re{\sum_{i=1}^m \mathring{\lambda}_i\bm{c}_i^\top\bm{h}} + \bm{h}^*A^*A\bm{h} \\
						&= f(\mathring{\bm{x}}) + \bm{h}^*A^*A\bm{h} \geq f(\mathring{\bm{x}})
					\end{align*}
				\end{proof}
	\chapter{発想, 技巧}
		\section{演算}
