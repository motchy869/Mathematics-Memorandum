\part{実解析}
	\chapter{表記}
		\begin{itemize}
			\item $\realNumbers_+ \coloneqq \{x\in\realNumbers|x\geq 0\}$ : 非負の実数全体の集合
			\item $\realNumbers_{++} \coloneqq \{x\in\realNumbers|x > 0\}$ : 正の実数全体の集合
	...
	\chapter{離散数学との関係}
		\section{DirichletのDiophantine近似定理}
			...
			\begin{proof}
				...
				区間$I_k \coloneqq [k/N,(k+1)/N)\;(k=0,\cdots,N-1)$を考える。
		...
		\section{諸定理}
			...
			\subsection{Jensenの不等式}
				...
				\begin{proof}
					...
					$w'_i\coloneqq w_i/(1-w_{n_0+1})$とする(最初の$n_0$個の重みを規格化したもの)と$w'_i>0,\;w'_1+\cdots+w'_{n_0}=1$だから、仮定より
					\begin{align*}
	...
	\chapter{多変数関数}
		...
		\section{勾配とHessian}
			...
			\subsection{勾配ベクトルが等値面の接平面と直交することの証明}
				\begin{shadebox}
					$n$変数関数$f(\bm{x}),\;(\bm{x}\coloneqq[x_1,\cdots,x_n]^\top)$の点$\bm{a}\coloneqq[a_1,\cdots,a_n]^\top$における勾配$\nabla f(\bm{a})$が$\bm{0}$でないとき、等値面$f(\bm{x})=f(\bm{a})$上の点$\bm{a}$における接平面と$\nabla f(\bm{a})$直交する。
				\end{shadebox}
				\begin{proof}
					\quad\par
					$\nabla f(\bm{a})\neq\bm{0}$だから$f_{x_1}(\bm{a}),\cdots,f_{x_n}(\bm{a})$のうち少なくとも1つは0でない。
					一般性を失わずに$f_{x_n}(\bm{a})\neq 0$であるとする(変数の添字をそのように付け替えれば良い)。
					このとき陰関数定理より点$\bm{a}'\coloneqq[a_1,\cdots,a_{n-1}]^\top$の近傍で$f(\bm{x})=f(\bm{a})$の陰関数$x_n=\varphi(\bm{x}'),\;(\bm{x}'=[x_1,\cdots,x_{n-1}]^\top)$のうち$\varphi(\bm{a}')=a_n$を満たすものが唯一存在する。
					...
					\[ W \coloneqq \Span{\set*{\left[0,\cdots,0,\underset{i\text{番目}}{1},0,\cdots,0,-\frac{f_{x_i}(\bm{a}')}{f_{x_n}(\bm{a}')}\right]^\top}{i=1,\cdots,n-1}} \]
		...
		\section{逆関数定理}
			...
			\begin{proof}
				...
				\par
				$F = [f_1,\cdots,f_{m+1}]^\top,\;\bm{u}=F(\bm{a}),\;\xhat \coloneqq [x_1,\cdots,x_m]^\top(\bm{a},\bm{u}\text{についても同様に定義})$とする。
				\par
				...
				\par
				$\fhat_i(\xhat) \coloneqq f_i([\xhat,h(\xhat)]^\top),\;\Fhat(\xhat) \coloneqq [\fhat_i(\xhat),\cdots,\fhat_m(\xhat)]^\top$とすると$\Fhat(\ahat) = \uhat$であり、
				...
				\[ \text{where}\quad \bm{p}_j \coloneqq \left(\partDeriv{\Fhat}{x_j}{}\right)(\ahat)\;(j=1,\cdots,m+1),\;\lambda_j \coloneqq \left(\partDeriv{h}{x_j}{}\right)(\ahat)\;(j=1,\cdots,m) \]
	...
	\chapter{重積分}
		...
		\section{\texorpdfstring{$C^1$}{C\string^1}級の変換による面積0の集合の像の面積もまた0}
			...
			\begin{proof}
				...
				第$d$軸方向の分割を$\Delta_d : x^{(d)}_i = a_d + \frac{i}{m}(b_d-a_d)\;(i=0,\cdots,m,\;d=1,\cdots,n)$とし、各小超直方体を$R_{i_1,\cdots,i_n} \coloneqq [x^{(1)}_{i_1-1}, x^{(1)}_{i_1}]\times\cdots\times[x^{(n)}_{i_n-1}, x^{(n)}_{i_n}]$で表す。
		...
		\section{関数\texorpdfstring{$f,g$}{f,g}が\texorpdfstring{$\realNumbers$}{R}上で有界,連続で\texorpdfstring{$\integ{-\infty}{\infty}{f(x)}{x},\;\integ{-\infty}{\infty}{g(x)}{x}$}{f,gの-∞から+∞に渡る積分}が絶対収束するなら畳み込み\texorpdfstring{$f*g$}{f*g}は\texorpdfstring{$\realNumbers$}{R}上で連続で絶対可積分である。}
			\begin{proof}
				...
				よって$h$を十分小さくすれば
				\[ |f(x-y+h) - f(x-y)| < \frac{\varepsilon}{3L} \quad \left(L \coloneqq \integ{-\infty}{\infty}{|g(y)|}{y} \right) \]
				となり、
				...
				(絶対可積分であること):
				\par
				$a>0$として$I(a) \coloneqq \integ{-a}{a}{|(f*g)(x)|}{x}$とすると$I(a)$は$a$に関して単調増加であり、次式が成り立つ。
				...
				これより
				\begin{align*}
					I(a) &\leq \integ{-\infty}{\infty}{\abs{g(y)}\integ{-a}{a}{\abs{f(x-y)}}{x}}{y} \leq \integ{-\infty}{\infty}{\abs{g(y)}M_1}{y} \quad \left( M_1 \coloneqq \integ{-\infty}{\infty}{\abs{f(x-y)}}{x} \right) \\
					&\leq M_1M_2 \quad \left( M_2 \coloneqq \integ{-\infty}{\infty}{\abs{g(y)}}{y} \right)
				\end{align*}
	...
	\chapter{線積分}
		...
		\section{計算法}
			...
			級数$S^{(2)} \coloneqq \sum_{i=1}^n f(\bm{\phi}(\mu_i))\phi_d'(\mu_i)(t_i-t_{i-1})$を考えると、これもまた$f$の$\Delta, \{\bm{\phi}(\mu_i)\}$に関するRiemann和であるから$f$の線積分可能性より、分割を細かくして$|\Delta|\to 0$とするとき$\integ{C}{}{f(\bm{x})}{x_d}$に収束する。
	...
	\chapter{Beta関数}
		...
		\section{多変量Beta関数}
			\begin{shadebox}
				...
				\begin{align*}
					B(\bm{\alpha}) &\coloneqq \integ{\Omega}{}{\prod_{i=1}^{n-1} x_i^{\alpha_i-1} \left(1-\sum_{i=1}^{n-1} x_i\right)^{\alpha_n-1}}{\Omega}, \quad \Omega \coloneqq \set*{[x_1,\cdots,x_{n-1}]^\top}{x_i\geq 0, \sum_{i=1}^{n-1} x_i\leq 1} \\
	...
	\chapter{微分と積分の関係}
		...
		\section{
			...
			\begin{proof}
				...
				\quad\par
				$I(x) \coloneqq \integ{0}{x}{f(s)g(x-s)}{s}$とおき、微分の定義に従い$\lim_{\dx\to 0}\frac{I(x+\dx)-I(x)}{\dx}$を計算する。
				\begin{align*}
		...
		\section{
			...
			\begin{shadebox}
				$\omega>0$とする。
				\[\Omega\coloneqq\{(x_1,\cdots,x_n)|x_1,\cdots,x_n\geq 0, x_1+\cdots+x_n \leq \omega\}\]
				\[\overline{\Omega}^+\coloneqq\{(x_1,\cdots,x_n)|x_1,\cdots,x_n\geq 0, x_1+\cdots+x_n = \omega\} \subset \Omega\]
				$f_1(x),\cdots,f_n(x)$を$\{x|x\geq 0\}$上の連続関数とし、$f(\bm{x})\coloneqq f_1(x_1)\cdots f_n(x_n),\quad F(\omega) \coloneqq \integ{\Omega}{}{f(\bm{x})}{\bm{x}}$とするとき、次が成り立つ。
			...
			\begin{proof}
				...
				\[g_1(\omega,x_1) \coloneqq \textcolor{red}{\int_{x_2=0}^{\omega-x_1+\domega}f_2(x_2)\cdots\int_{x_n=0}^{\omega-x_1-\cdots-x_{n-1}+\domega}f_n(x_n)\d{x_n}\cdots\d{x_2}}\]
				とおけば
				\begin{align*}
				...
				\[g_2(\omega,x_1,x_2) \coloneqq \int_{x_3=0}^{\omega-x_1-x_2+\domega}f_3(x_3)\cdots\int_{x_n=0}^{\omega-x_1-\cdots-x_{n-1}+\domega}f_n(x_n)\d{x_n}\cdots\d{x_3}\]
				とおけば
				\begin{align*}
		...
		\section{
			...
			\label{重積分の微分の系}
			\begin{shadebox}
				\[\Omega\coloneqq\{(x_1,\cdots,x_n)|x_1+\cdots+x_n \leq \omega\}\]
				\[\overline{\Omega}^+\coloneqq\{(x_1,\cdots,x_n)|x_1+\cdots+x_n = \omega\} \subset \Omega\]
				$f_1(x),\cdots,f_n(x)$を$\mathbb{R}$上の連続関数とし、$f(\bm{x})\coloneqq f_1(x_1)\cdots f_n(x_n),\quad F(\omega) \coloneqq \integ{\Omega}{}{f(\bm{x})}{\bm{x}}$とするとき、次が成り立つ。
				\[\deriv{F(\omega)}{\omega}{} = \integ{\overline{\Omega}^+}{}{f(\bm{x})}{\bm{x}}\]
