\part{実解析}
	...
	\chapter{重積分}
		...
		\section{関数\texorpdfstring{$f,g$}{f,g}が\texorpdfstring{$\realNumbers$}{R}上で有界,連続で\texorpdfstring{$\integ{-\infty}{\infty}{f(x)}{x},\;\integ{-\infty}{\infty}{g(x)}{x}$}{f,gの-∞から+∞に渡る積分}が絶対収束するなら畳み込み\texorpdfstring{$f*g$}{f*g}は\texorpdfstring{$\realNumbers$}{R}上で連続で絶対可積分である。}
			...
			\end{proof}
		\section{諸公式}
			\subsection{\texorpdfstring{$n$}{n}次元の単体の体積\texorpdfstring{$\int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_n=0}^{a-x_1- \dotsb - x_{n-2}} \mathrm{d}x_n \cdots \mathrm{d}x_2\mathrm{d}x_1 = a^n/n!\;(a>0)$}{}}
			\begin{proof}
				\begin{align*}
					&\phantom{=} \int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_n=0}^{a-x_1- \dotsb - x_{n-2}} \mathrm{d}x_n \cdots \mathrm{d}x_2\mathrm{d}x_1 \\
					&= \int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_{n-1}=0}^{a-x_1- \dotsb - x_{n-3}} (a-x_1- \dotsb - x_{n-1}) \mathrm{d}x_{n-1} \cdots \mathrm{d}x_2\mathrm{d}x_1 \\
					&= \frac{1}{2}\int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_{n-2}=0}^{a-x_1-\dotsb-x_{n-4}} (a-x_1- \dotsb - x_{n-2})^2 \mathrm{d}x_{n-2} \cdots \mathrm{d}x_2\mathrm{d}x_1 \\
					&= \dotsb \\
	...
	\chapter{微分と積分の関係}
		...
		\section{
			...
			\small{
				\texorpdfstring{$\derivLong{\left(\integ{x_1=0}{x}{\cdots\integ{x_n=0}{x-x_1-\dotsb-x_{n-1}}{f_1(x_1)\cdots f_n(x_n)}{x_n}\cdots}{x_1}\right)}{x}{}$}{} \texorpdfstring{\\}{}
				\texorpdfstring{$= \int_{x_1=0}^x\cdots\int_{x_{n-1}=0}^{x-x_1-\dotsb -x_{n-2}} f_1(x_1)\cdots f_{n-1}(x_{n-1})f_n(x-x_1-\dotsb-x_{n-1}) \mathrm{d}x_{n-1}\cdots\mathrm{d}x_{1}$}{}
			}
		}
			\label{重積分の微分}
			\begin{shadebox}
				...
				要するに、領域$x_1,\dotsc,x_n\geq 0$の超平面$x_1+\dotsb+x_n=\omega$の原点側の部分における$f_1(x_1)\cdots f_n(x_n)$の積分を$\omega$で微分したものは領域$x_1,\dotsc,x_n\geq 0$と超平面の共通部分上での積分に等しい。\\
				\\
			...
			\begin{proof}
				...
				\begin{align*}
					F(\omega+\domega) &= \integ{x_1=0}{\omega}{\cdots\integ{x_n=0}{\omega-x_1-\dotsb-x_{n-1}}{f_1(x_1)\cdots f_n(x_n)}{x_n}\cdots}{x_1} \\
					...
					& \textcolor{red}{\left(\int_{x_n=0}^{\omega-x_1-\dotsb-x_{n-1}}+\int_{x_n=\omega-x_1-\dotsb-x_{n-1}}^{\omega-x_1-\dotsb-x_{n-1}+\domega}\right)f_n(x_n)\d{x_n}\cdots\d{x_2}}\d{x_1}
				\end{align*}
				...
				\[g_1(\omega,x_1) \coloneqq \textcolor{red}{\int_{x_2=0}^{\omega-x_1+\domega}f_2(x_2)\cdots\int_{x_n=0}^{\omega-x_1-\dotsb-x_{n-1}+\domega}f_n(x_n)\d{x_n}\cdots\d{x_2}}\]
				とおけば
				\begin{align*}
				...
				\[g_2(\omega,x_1,x_2) \coloneqq \int_{x_3=0}^{\omega-x_1-x_2+\domega}f_3(x_3)\cdots\int_{x_n=0}^{\omega-x_1-\dotsb-x_{n-1}+\domega}f_n(x_n)\d{x_n}\cdots\d{x_3}\]
				とおけば
				\begin{align*}
				...
				\begin{align*}
					...
					&= \int_{x_1=0}^{\omega}f_1(x_1)\cdots\int_{x_{n-1}=0}^{\omega-x_1-\dotsb-x_{n-2}}f_{n-1}(x_{n-1}) \\
					& \left(\int_{x_n=0}^{\omega-x_1-\dotsb-x_{n-1}}+\int_{x_n=\omega-x_1-\dotsb-x_{n-1}}^{\omega-x_1-\dotsb-x_{n-1}+\domega}\right)f_n(x_n)\d{x_n}\d{x_{n-1}}\cdots\d{x_1} \\
					&= F(\omega) + \underbrace{\int_{x_1=0}^{\omega}f_1(x_1)\cdots\int_{x_{n-1}=0}^{\omega-x_1-\dotsb-x_{n-2}}f_{n-1}(x_{n-1})\textcolor{red}{\int_{x_n=\omega-x_1-\dotsb-x_{n-1}}^{\omega-x_1-\dotsb-x_{n-1}+\domega}f_n(x_n)\d{x_n}}\d{x_{n-1}}\cdots\d{x_1}}_{=:h_{n2}(\omega,\domega)}
				\end{align*}
				...
				\begin{align*}
					\lim_{\domega\to 0}\frac{h_{n2}(\omega,\domega)}{\domega} &= \int_{x_1=0}^{\omega}f_1(x_1)\cdots\int_{x_{n-1}=0}^{\omega-x_1-\dotsb-x_{n-2}}f_{n-1}(x_{n-1})f_n(\theta_n)\d{x_{n-1}}\cdots\d{x_1} \\
					&= \int_{x_1=0}^{\omega}f_1(x_1)\cdots\int_{x_{n-1}=0}^{\omega-x_1-\dotsb-x_{n-2}}f_{n-1}(x_{n-1})f_n(\omega-x_1-\dotsb-x_{n-1})\d{x_{n-1}}\cdots\d{x_1} \\
		...
		\section{
			系\texorpdfstring{\\}{}
			\small{
				\texorpdfstring{$\derivLong{\left(\integ{x_1=-\infty}{\infty}{\cdots\integ{x_n=-\infty}{x-x_1-\dotsb-x_{n-1}}{f_1(x_1)\cdots f_n(x_n)}{x_n}\cdots}{x_1}\right)}{x}{}$\\}{}
				\texorpdfstring{$= \integ{x_1=-\infty}{\infty}{\cdots\integ{x_{n-1}=-\infty}{x-x_1-\dotsb-x_{n-2}}{f_1(x_1)\cdots f_{n-1}(x_{n-1})f_n(x-x_1-\dotsb-x_{n-1})}{x_{n-1}}\cdots}{x_1}$}{}
			}
		}
			\label{重積分の微分の系}
			\begin{shadebox}
				...
				\[\deriv{F(\omega)}{\omega}{} = \integ{\overline{\Omega}^+}{}{f(\bm{x})}{\bm{x}}\]
				要するに、超平面$x_1+\dotsb+x_n=\omega$の負領域における$f_1(x_1)\cdots f_n(x_n)$の積分を$\omega$で微分したものは超平面上での積分に等しい。\\
				\\
