\part{実解析}
	\chapter{表記}
		...
		\end{itemize}
	\chapter{床関数と天井関数}
		\section{$\forall x\in\realNumbers,\;\floor{-x} = -\ceil{x},\;\ceil{-x} = -\floor{x}$}
			\begin{proof}
				\quad\\
				前者は次のようにして示せる。
			...
			\end{proof}
		\section{$^\forall x,y\in\realNumbers,\;\floor{x}+\floor{y}\leq\floor{x+y},\;\ceil{x}+\ceil{y}\geq\ceil{x+y}$}
			\begin{proof}
				\quad\\
