\part{実解析}
	\chapter{表記}
		\begin{itemize}
			\item $\realSpace_+ := \{x\in\realSpace|x\geq 0\}$ : 非負の実数全体の集合
			\item $\realSpace_{++} := \{x\in\realSpace|x > 0\}$ : 正の実数全体の集合
			\item $\bm{1}_n$ : 要素が全て1である$n$次元列ベクトル
			\item $\bm{0}_n$ : 要素が全て0である$n$次元列ベクトル
			\item $\bm{x}[i]$ : 列(行)ベクトルの第$i$成分
			\item $\matEntry{A}{i}{j}$ : 行列$A$の第$(i,j)$成分
			...
			\item $A_{i,j}$ : 行列$A$の第$i$行$j$列を取り除いた行列
		\end{itemize}
	\chapter{床関数と天井関数}
		\section{$\forall x\in\realSpace,\;\floor{-x} = -\ceil{x},\;\ceil{-x} = -\floor{x}$}
			\begin{proof}
				\quad\\
				前者は次のようにして示せる。
			...
			\end{proof}
		\section{$^\forall x,y\in\realSpace,\;\floor{x}+\floor{y}\leq\floor{x+y},\;\ceil{x}+\ceil{y}\geq\ceil{x+y}$}
			\begin{proof}
				\quad\\
	...
	\chapter{乗積}
		\section{$a_i\geq 0\;(i=1,2,\cdots),\;\sum_{i=1}^\infty (1-a_i) = \infty \Rightarrow \prod_{i=1}^\infty a_i = 0$}
			\begin{proof}
				...
				$b_i=-\log a_i$とすると$a_i=e^{-b_i}$であり、$\forall x\in\realSpace,\;1-e^{-x}\leq x$より$1-a_i=1-e^{-b_i}\leq b_i$なので次式が成り立つ。
	...
	\chapter{代数方程式}
		\section{諸定理}
			\subsection{係数が次数に関して対称な奇数次の多項式$f(x)$を$(x+1)$で除したものも係数が次数に関して対称になる}
				\begin{proof}
					\quad\par
					...
					次に$x,x^2,\cdots,x^{n-1}$の項の係数に関しては$b_{i-1} + b_i = a_i = a_{n-i} = b_{n-1-i} + b_{n-i}$すなわち$b_i - b_{n-1-i} = b_{(n-1)-(i-1)} - b_{i-1}$となる。
					既に見た$b_0 = b_{n-1}$から始めて帰納的に示せる。
				\end{proof}
			\subsection{実係数$n$次方程式が複素数解$s=\alpha+i\beta$を解に持つならばその複素共役$s^* = \alpha-i\beta$もまた解である}
				\begin{shadebox}
				...
				\begin{proof}
					...
					\[x^n + \sum_{k=n-1}^{0}{c_ix^k} = 0,\quad c_k \in \realSpace\]
					と書ける。$s=\alpha+i\beta,\;\;\alpha,\beta\in\realSpace$がこれの解であるから当然
	...
	\chapter{微分}
		...
		\section{soft-step 関数}
			...
			\subsection{$C^{n-1}$級の soft-step 関数}
				$\sigma>0, n\in\naturalNumbers$とする。次の関数は$\realSpace$上で$C^{n-1}$級である。
				\[
					f(x) =
					\begin{cases}
						0 & (x < -\sigma) \\
	...
	\chapter{凸関数}
		\section{諸定理}
			...
			\subsection{Jensenの不等式}
				...
				\end{proof}
			\subsection{重み付き相加,相乗平均の関係とヤングの不等式}
				\begin{shadebox}
				...
				\end{proof}
				この不等式には変形版があり、\textbf{ヤングの不等式}と呼ばれている。
	...
	\chapter{諸定理}
		...
		\section{ヘルダーの不等式}
			...
			\begin{proof}
				\quad\par
				ヤングの不等式より$a,b\geq 0$に対して$ab \leq a^p/p + b^q/q$が成り立つ。
				この式で$a=|x_i|/\norm{\bm{x}}_p,b=|y_i|/\norm{\bm{y}}_q$とすると
	...
	\chapter{多変数関数}
		\section{Jacobi行列}
			\subsection{$J_{G\circ F}=J_GJ_F$}
				...
				\begin{shadebox}
					$F:\realSpace^l\to\realSpace^m,\;\bm{x}\mapsto\bm{y}(\bm{x})$と$G:\realSpace^m\to\realSpace^n,\;\bm{y}\mapsto\bm{z}(\bm{y})$のJacobi行列を$J_F,J_G$とすると、$F,G$の合成写像$G\circ F$のJacobi行列は$J_{G\circ F}=J_GJ_F$である。
				\end{shadebox}
				\begin{proof}
					\[ J_{G\circ F}[i][j] = \left(\partDeriv{z_i}{x_j}{}\right)(\bm{x}) = \sum_{k=1}^n \left(\partDeriv{z_i}{y_k}{}\right)(\bm{y})\left(\partDeriv{y_k}{x_j}{}\right)(\bm{x}) = J_G[i][*]J_F[*][j] \]
				\end{proof}
			\subsection{$J_{\bm{f}^\top\bm{g}} = \bm{g}^\top J_\bm{f} + \bm{f}^\top J_\bm{g}$}
			\begin{shadebox}
				$\bm{f},\bm{g}:\realSpace^m\to\realSpace^n$のJacobi行列を$J_\bm{f},J_\bm{g}$とすると、$J_{\bm{f}^\top \bm{g}}=\bm{g}^\top J_\bm{f} + \bm{f}^\top J_\bm{g}$である。
			\end{shadebox}
			\begin{proof}
				\begin{align*}
		...
		\section{勾配とHessian}
			\subsection{$\nabla(G\circ F) = \nabla F \nabla G$}
				\begin{shadebox}
					$F:\realSpace^l\to\realSpace^m,\;\bm{x}\mapsto\bm{y}(\bm{x})$と$G:\realSpace^m\to\realSpace^n,\;\bm{y}\mapsto\bm{z}(\bm{y})$に対して$F,G$の合成写像$G\circ F$の勾配は$\nabla(G\circ F) = \nabla F \nabla G$である。
				\end{shadebox}
				\begin{proof}
			...
			\subsection{$\nabla_\bm{xy}^2 f = (\nabla_\bm{yx}^2 f)^\top$}
				\begin{shadebox}
					$\bm{x}\in\realSpace^{n_1}, \bm{y}\in\realSpace^{n_2}$とし、$f:(\bm{x},\bm{y})\in\realSpace^{n_1+n_2}\mapsto f(\bm{x},\bm{y})\in\realSpace$とするとき、次が成り立つ。
					\[ \nabla_\bm{xy}^2 f = (\nabla_\bm{yx}^2 f)^\top \]
				\end{shadebox}
				\begin{proof}
		...
		\section{逆関数定理}
			\begin{shadebox}
				$F\in C^1(\realSpace^n\to\realSpace^n),\;\bm{x}\mapsto\bm{y}(\bm{x})$のJacobi行列を$J_F(\bm{x})$とする。
			...
			\begin{proof}
				...
				\par
				$J_F(\bm{a})\in\realSpace^{(m+1)\times(m+1)}$は正則なので$m+1$行目の成分の少なくとも1つはは0でない。
				一般性を失わず第$m+1,m+1$成分$\left(\partDeriv{f_{m+1}}{x_{m+1}}{}\right)(\bm{a})$が0でないとすると、陰関数定理より関数$h\in C^1(\realSpace^m\to\realSpace$)で$f_{m+1}([\xhat^\top,h(\xhat)]^\top) = u_{m+1},\;h(\ahat) = a_{m+1}$を満たすものが$\ahat$の近傍で唯一存在する。
				...
				帰納法の仮定より$J_{\Fhat}(\ahat)$が正則であれば$\uhat(=\Fhat(\ahat))$の近傍で$\Fhat$の逆写像$\Fhat^{-1}\in C^1(\realSpace^m\to\realSpace^m)$が存在して$\uhat$に対応する$\ahat$、さらには$a_{m+1}=h(\ahat)$も唯一に定まる。
				すなわち$F^{-1}$が存在する。そこで$|J_{\Fhat}(\ahat)|\neq 0$を示す。列に着目すると
				\par
				\[ J_{\Fhat}(\ahat)[*][j] = \left(\partDeriv{\Fhat}{x_j}{}\right)(\ahat) + \left(\partDeriv{\Fhat}{x_{m+1}}{}\right)(h(\ahat))\left(\partDeriv{h}{x_j}{}\right)(\ahat) = \bm{p}_j + \lambda_j\bm{p}_{m+1} \]
			...
			\end{proof}
		\section{2次形式}
			\subsection{$^\forall A\in\realSpace^{n\times n},\bm{x}\in\realSpace^n,\;^\exists$対称行列$B\in\realSpace^{n\times n} \text{ s.t. } \bm{x}^\top A\bm{x} = \bm{x}^\top B\bm{x}$}
				\begin{proof}
					\quad\par
	...
	\chapter{測度}
		...
		\section{Lebesgue測度}
			...
			\subsection{有界集合$A$に対して$\mLebO{A}\leq\mJo{A}$}
				...
				\end{proof}
			\subsection{有界閉集合$A\subset\realSpace^n$に対して$\mLebO{A}=0 \iff \mJordan{A}=0$}
				\begin{proof}
					\quad\par
					...
					$\bigcup_{n=1}^\infty \textrm{Int}(2E_n)$は$A$の開被覆であり、$A$は$\realSpace^n$の有界閉集合なのでコンパクトであるから、ある$N\in\naturalNumbers$が存在して$A\subset \bigcup_{n=1}^N \textrm{Int}(2E_n)$となる。
				...
				\end{proof}
			\subsection{$A\subset\realSpace^n$とする。任意の直方体$E$に対して$\mLebO{A\cap E} + \mLebO{A^\complement\cap E} = \mLebO{E} \iff$任意の$B\subset\realSpace^n$に対して$\mLebO{A\cap B} + \mLebO{A^\complement\cap B} = \mLebO{B}$}
				\begin{proof}
					\quad\par
				...
				\end{proof}
			\subsection{有界集合$A\subset\realSpace^n$がJordan可測ならばLebesgue可測であり、$\mLeb{A}=\mJordan{A}$}
				\begin{proof}
					\quad\par
				...
				\end{proof}
			\subsection{$A_1,A_2\in\realSpace^n$がLebesgue可測なら$A_1\cup A_2$もLebesgue可測}
				\begin{proof}
					\quad\par
					$B\in\realSpace^n$を任意の集合とする。
					...
					\end{align*}
				\end{proof}
			\subsection{系: $A_1,\cdots,A_n\in\realSpace^n$がLebesgue可測なら$A_1\cup A_2\cup\cdots\cup A_n$もLebesgue可測}
				\begin{proof}
					直前の定理を繰り返し用いる。
				\end{proof}
			\subsection{Lebesgue可測集合$A_1,A_2\in\realSpace^n$が非交差的なら$\mLeb{A_1\sqcup A_2} = \mLeb{A_1} + \mLeb{A_2}$}
				\begin{proof}
					\quad\par
					...
					$B$は任意なので$B=\realSpace^n$とすると$\mLebO{A_1\sqcup A_2} = \mLebO{A_1} + \mLebO{A_2}$となるが、$A_1,A_2, A_1\sqcup A_2$が可測であるので外測度=測度であるから結局
					\[ \mLeb{A_1\sqcup A_2} = \mLeb{A_1} + \mLeb{A_2} \]
				\end{proof}
			\subsection{系: Lebesgue測度の有限加法性}
				Lebesgue可測集合$A_1,\cdots,A_n\in\realSpace^n$が非交差的なら$\mLeb{\bigsqcup_{i=1}^n A_i} = \sum_{i=1}^n \mLeb{A_i}$
				\begin{proof}
					直前の定理を繰り返し用いる。
				\end{proof}
			\subsection{Lebesgue測度の$\sigma$--加法性}
				Lebesgue可測集合$A_1,A_2,\cdots\in\realSpace^n$が非交差的なら$\mLeb{\bigsqcup_{i=1}^\infty A_i} = \sum_{i=1}^\infty \mLeb{A_i}$
				\begin{proof}
					\quad\par
					$B_n := \bigsqcup_{i=1}^n A_i,\;B := \bigsqcup_{i=1}^\infty A_i$とする。
					任意の集合$S\in\realSpace^n$に対して
					...
					\begin{align*}
						...
						&\geq \mLebO{S}
					\end{align*}
					よって$B$はLebesgue可測であり$\mLebO{S\cap B} = \sum_{i=1}^\infty \mLebO{S\cap A_i}$である。$S=\realSpace^n$とすると
	...
	\chapter{重積分}
		\section{平均値の定理}
			\begin{shadebox}
				$\Omega\in\realSpace^n$を体積確定の有界閉領域、$f:\Omega\to\realSpace$を連続関数とすると、$^\exists\bm{x}'\st\integ{\Omega}{}{f(\bm{x})}{\bm{x}} = f(\bm{x}')|\Omega|$
			\end{shadebox}
			\begin{proof}
				\quad\par
		...
		\section{$C^1$級の変換による面積0の集合の像の面積もまた0}
			\begin{shadebox}
				閉超直方体$R\subset\realSpace^n$を含む開集合$D$上で定義された$C^1$級の\textcolor{red}{変換}$F:D\to\realSpace^n$を考える。
				集合$C\subset\realSpace^n$の面積が0であるとき、$F(C)\subset\realSpace^n$の面積もまた0である。
			\end{shadebox}
			\begin{proof}
				\quad\par
			...
			\end{proof}
		\section{関数$f,g$が$\realSpace$上で有界,連続で$\integ{-\infty}{\infty}{f(x)}{x},\;\integ{-\infty}{\infty}{g(x)}{x}$が絶対収束するなら畳み込み$f*g$は$\realSpace$上で連続で絶対可積分である。}
			\begin{proof}
				\quad\\
				(連続であること):
				\par
				$\varepsilon>0,\;x\in\realSpace,\;h\in(-1,1)$を任意にとる。
				\begin{align*}
				...
				となる。$f$は$\realSpace$上で連続だから特に$[x-a-1,x+a+1]$上で一様連続である。
				よって$h$を十分小さくすれば
	...
	\chapter{線積分}
		\section{定義}
			$\realSpace^n$上の有界関数$f$と区分的に滑らかな有向曲線$C$に対して$x_d\;(d=1,\cdots,n)$軸方向の線積分を以下のようにして定義する。
			\par
		...
		\section{計算法}
			$f$が$C$に関して線積分可能であるとし、パラメータ$t$を用いて$\bm{x}=\bm{\phi}(t)=[\phi_1(t),\cdots,\phi_n(t)]^\top,\; \bm{\phi} \in C^1(\realSpace\to\realSpace^n),\;t:a\to b$と表せる場合は次が成り立つ。
	...
	\chapter{図形への応用}
		\section{ある超平面から距離$d$だけ離れた超平面の方程式}
			$n\in\naturalNumbers,\;\bm{w}\in\realSpace^n\neq\bm{0},\;c\in\realSpace$とする。
			写像$f:\bm{x}\in\realSpace^n\mapsto \bm{w}^\top\bm{x} + c \in \realSpace$が定める$\realSpace^n$上の超平面$f(\bm{x})=0$
		...
		\section{$n$次元単位球の体積$V_n$は$\pi^{n/2}(\GammaFunc{1+n/2})^{-1}$}
			\begin{proof}
				\quad\par
				$\Omega(n,r)\;(r\in\realSpace) := \set{\bm{x}\in\realSpace^n}{\norm{\bm{x}}_{2}\leq r}$とすると
				\begin{align*}
