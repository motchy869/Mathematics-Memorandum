\part{実解析}
	...
	\chapter{凸関数}
		...
		\section{凸関数の例}
			...
			\subsection{$\|\sum_{i=1}^N X_i\bm{a}_i + \bm{b}\|_2^2\;(\bm{a}_i, \bm{b} \in \complexNumbers^n,\;X_i \in \complexNumbers^{m\times n})$は$X_1,\cdots,X_N$に関して凸である。}
				\begin{proof}
					\quad\par
					$T_X := (X_1,\cdots,X_N),\;T_Y := (Y_1,\cdots,Y_N),\;\lambda \in [0,1]$とし、$f(T_X) := \|\sum_{i=1}^N X_i\bm{a}_i + \bm{b}\|_2^2$とする。
					\begin{align*}
	...
	\chapter{重積分}
		...
		\section{関数$f,g$が$\realNumbers$上で有界,連続で$\integ{-\infty}{\infty}{f(x)}{x},\;\integ{-\infty}{\infty}{g(x)}{x}$が絶対収束するなら畳み込み$f*g$は$\realNumbers$上で連続で絶対可積分である。}
			...
			\end{proof}
		\section{諸公式}
			\subsection{$\int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_n=0}^{a-x_1- \cdots - x_{n-2}} \mathrm{d}x_n \cdots \mathrm{d}x_2\mathrm{d}x_1 = a^n/n!\;(a>0)$}
			\begin{proof}
				\begin{align*}
					&\phantom{=} \int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_n=0}^{a-x_1- \cdots - x_{n-2}} \mathrm{d}x_n \cdots \mathrm{d}x_2\mathrm{d}x_1 \\
					&= \int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_{n-1}=0}^{a-x_1- \cdots - x_{n-3}} (a-x_1- \cdots - x_{n-1}) \mathrm{d}x_{n-1} \cdots \mathrm{d}x_2\mathrm{d}x_1 \\
					&= \frac{1}{2}\int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_{n-2}=0}^{a-x_1-\cdots-x_{n-4}} (a-x_1- \cdots - x_{n-2})^2 \mathrm{d}x_{n-2} \cdots \mathrm{d}x_2\mathrm{d}x_1 \\
					&= \cdots \\
                    &= \frac{1}{2\times 3\times\cdots(n-1)}\int_{x_1=0}^a (a-x_1)^{n-1} \mathrm{d}x_1 \\
                    &= \frac{a^n}{n!}
				\end{align*}
			\end{proof}
	\chapter{線積分}
	...
	\chapter{最適化への応用}
		\section{狭義凸関数の線形制約下での最小化}
			\begin{shadebox}
				...
				$\bm{c}_i \in \realNumbers^n, b_i \in \realNumbers\;(i=1,\cdots,m)$とし、$g_i: \bm{x} \in \realNumbers^n \mapsto \bm{c}_i^\top\bm{x} + b \in \realNumbers$とする。
				$\bm{\lambda} \in \realNumbers^m$とし、Lagrange関数$F$を$F(\bm{x},\bm{\lambda}) := f(\bm{x}) + \sum_{i=1}^m \lambda_i g_i(\bm{x})$とする。
				$\bm{x}_0, \bm{\lambda}_0$を$F$の停留点、すなわち$\nabla_\bm{x}F(\bm{x}_0, \bm{\lambda}_0) = \bm{0}$とすると$f$は$g_i(\bm{x}) = 0\;(i=1,\cdots,m)$なる拘束条件の下、$\bm{x}_0$で最小値をとる。
			\end{shadebox}
			\begin{proof}
				\quad\par
				$\bm{h} \in \realNumbers^n \neq \bm{0}$とし、$g_i(\bm{x} + \bm{h}) = 0\;(i=1,\cdots,m)$を満たすとする。
				...
				$g_i(\bm{x} + \bm{h}) = 0\;(i=1,\cdots,m)$より$\bm{c}_i^\top\bm{h} = \bm{0}$であるから
