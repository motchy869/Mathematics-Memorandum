\part{実解析}
	...
	\chapter{離散数学との関係}
		\section{DirichletのDiophantine近似定理}
			...
			\begin{proof}
				...
				区間$I_k \coloneqq [k/N,(k+1)/N)\;(k=0,\dotsc,N-1)$を考える。
	...
	\chapter{凸関数}
		\section{諸定理}
			...
			\subsection{重み付き相加,相乗平均の関係とYoungの不等式}
				\begin{shadebox}
					$w_1,\dotsc,w_n\geq 0,\;w_1+\cdots+w_n=1,\;x_1,\dotsc,x_n\geq 0$とするとき、次式が成り立つ。
				...
				\begin{shadebox}
					$p_1,\dotsc,p_n>1,\;1/p_1+\cdots+1/p_n=1,\;x_1,\dotsc,x_n\geq 0$とするとき、次式が成り立つ。
		...
		\section{逆関数定理}
			...
			\begin{proof}
				...
				\begin{align*}
					...
					&+ \left. \sum_{i=1}^m (-1)^{m+i}\left(\partDeriv{f_{m+1}}{x_i}{}\right)(\bm{a})\left|\bm{p}_1,\cdots\bm{p}_{i-1},\bm{p}_{i+1},\dotsc,\bm{p}_{m+1}\right|\right]
				\end{align*}
		...
		\section{Lebesgue測度}
			...
			\subsection{\texorpdfstring{$A_1,A_2\subset\realNumbers^n$}{A1,A2⊂R\string^n}がLebesgue可測なら\texorpdfstring{$A_1\cup A_2$}{A1とA2の和集合}もLebesgue可測}
				\begin{proof}
					...
					\end{align*}
				\end{proof}
			\subsection{系: \texorpdfstring{$A_1,\dotsc,A_n\subset\realNumbers^n$}{A1,...,An⊂R\string^n}がLebesgue可測なら\texorpdfstring{$A_1\cup A_2\cup\cdots\cup A_n$}{A1,...,Anの和集合}もLebesgue可測}
				\begin{proof}
					直前の定理を繰り返し用いる。
				\end{proof}
	...
	\chapter{重積分}
		...
		\section{\texorpdfstring{$C^1$}{C\string^1}級の変換による面積0の集合の像の面積もまた0}
			...
			\begin{proof}
				...
				第$d$軸方向の分割を$\Delta_d : x^{(d)}_i = a_d + \frac{i}{m}(b_d-a_d)\;(i=0,\dotsc,m,\;d=1,\dotsc,n)$とし、各小超直方体を$R_{i_1,\dotsc,i_n} \coloneqq [x^{(1)}_{i_1-1}, x^{(1)}_{i_1}]\times\cdots\times[x^{(n)}_{i_n-1}, x^{(n)}_{i_n}]$で表す。
	...
	\chapter{Beta関数}
		...
		\section{多変量Beta関数}
			\begin{shadebox}
				...
				\begin{align*}
					B(\bm{\alpha}) &\coloneqq \integ{\Omega}{}{\prod_{i=1}^{n-1} x_i^{\alpha_i-1} \left(1-\sum_{i=1}^{n-1} x_i\right)^{\alpha_n-1}}{\Omega}, \quad \Omega \coloneqq \set*{[x_1,\dotsc,x_{n-1}]^\top}{x_i\geq 0, \sum_{i=1}^{n-1} x_i\leq 1} \\
	...
	\chapter{微分と積分の関係}
		...
		\section{
			...
			\begin{shadebox}
				$\omega>0$とする。
				\[\Omega\coloneqq\{(x_1,\dotsc,x_n)|x_1,\dotsc,x_n\geq 0, x_1+\cdots+x_n \leq \omega\}\]
				\[\overline{\Omega}^+\coloneqq\{(x_1,\dotsc,x_n)|x_1,\dotsc,x_n\geq 0, x_1+\cdots+x_n = \omega\} \subset \Omega\]
				$f_1(x),\dotsc,f_n(x)$を$\{x|x\geq 0\}$上の連続関数とし、$f(\bm{x})\coloneqq f_1(x_1)\cdots f_n(x_n),\quad F(\omega) \coloneqq \integ{\Omega}{}{f(\bm{x})}{\bm{x}}$とするとき、次が成り立つ。
				\[\deriv{F(\omega)}{\omega}{} = \integ{\overline{\Omega}^+}{}{f(\bm{x})}{\bm{x}}\]
				要するに、領域$x_1,\dotsc,x_n\geq 0$の超平面$x_1+\cdots+x_n=\omega$の原点側の部分における$f_1(x_1)\cdots f_n(x_n)$の積分を$\omega$で微分したものは領域$x_1,\dotsc,x_n\geq 0$と超平面の共通部分上での積分に等しい。\\
				\\
		...
		\section{
			...
			\label{重積分の微分の系}
			\begin{shadebox}
				\[\Omega\coloneqq\{(x_1,\dotsc,x_n)|x_1+\cdots+x_n \leq \omega\}\]
				\[\overline{\Omega}^+\coloneqq\{(x_1,\dotsc,x_n)|x_1+\cdots+x_n = \omega\} \subset \Omega\]
				$f_1(x),\dotsc,f_n(x)$を$\mathbb{R}$上の連続関数とし、$f(\bm{x})\coloneqq f_1(x_1)\cdots f_n(x_n),\quad F(\omega) \coloneqq \integ{\Omega}{}{f(\bm{x})}{\bm{x}}$とするとき、次が成り立つ。
				\[\deriv{F(\omega)}{\omega}{} = \integ{\overline{\Omega}^+}{}{f(\bm{x})}{\bm{x}}\]
