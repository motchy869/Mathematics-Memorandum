	...
	\chapter{変数変換}
		\section{三角関数}
			\subsection{$x^2+y^2=1\iff x=\cos{\theta},y=\sin{\theta}$}
				...
				\end{proof}
		\section{極座標表示}
			\subsection{極方程式から曲線を導くときの注意 : $r\theta$空間の2つの異なる集合から$xy$空間へ写像した2つの集合の排他的論理和が$\emptyset$なら$x,y$空間において両者の違いは無い}
				...
				そこで$S_2 := \set{(r,\theta)}{\theta \in [-\pi,\pi], r=\sqrt{\cos 2\theta}}$とすると、$S,S_2$それぞれを$x,y$空間に写像してできる2つの曲線の排他的論理和は$\emptyset$である。
