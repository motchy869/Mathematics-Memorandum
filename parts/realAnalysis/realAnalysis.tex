\part{実解析}
		...
		\section{Lebesgue測度}
			...
			\subsection{\texorpdfstring{$A_1,A_2\subset\realNumbers^n$}{A1,A2⊂R\string^n}がLebesgue可測なら\texorpdfstring{$A_1\cup A_2$}{A1とA2の和集合}もLebesgue可測}
				\begin{proof}
					...
					\end{align*}
				\end{proof}
			\subsection{系: \texorpdfstring{$A_1,\cdots,A_n\subset\realNumbers^n$}{A1,...,An⊂R\string^n}がLebesgue可測なら\texorpdfstring{$A_1\cup A_2\cup\cdots\cup A_n$}{A1,...,Anの和集合}もLebesgue可測}
				\begin{proof}
					直前の定理を繰り返し用いる。
				\end{proof}
	...
	\chapter{積分}
		\section{変数変換 : \texorpdfstring{$\integ{x(t_0)}{x(t)}{f(x)}{x} = \integ{t_0}{t}{f(x)x'(t)}{t}$}{∫\_x(t0)\string^x(t) f(x)dx = ∫\_t0\string^t f(x)x'(t)dt}}
			\begin{shadebox}
			...
			\begin{proof}
				...
				以上より両者は等しい。
			\end{proof}
		\section{FKG不等式 : 可積分な単調増加関数\texorpdfstring{$f,g$}{f,g}に対して\texorpdfstring{$\left(\integ{0}{1}{f(x)}{x}\right)\left(\integ{0}{1}{g(x)}{x}\right) \leq \integ{0}{1}{f(x)g(x)}{x}$}{(fの定積分)×(gの定積分)≦f×gの定積分}}
			\begin{proof}
	...
	\chapter{重積分}
		...
		\section{\texorpdfstring{$C^1$}{C\string^1}級の変換による面積0の集合の像の面積もまた0}
			...
			\end{proof}
		\section{関数\texorpdfstring{$f,g$}{f,g}が\texorpdfstring{$\realNumbers$}{R}上で有界,連続で\texorpdfstring{$\integ{-\infty}{\infty}{f(x)}{x},\;\integ{-\infty}{\infty}{g(x)}{x}$}{f,gの-∞から+∞に渡る積分}が絶対収束するなら畳み込み\texorpdfstring{$f*g$}{f*g}は\texorpdfstring{$\realNumbers$}{R}上で連続で絶対可積分である。}
			\begin{proof}
				\quad\\
				(連続であること):
				\par
			...
			\end{proof}
		\section{諸公式}
			\subsection{\texorpdfstring{$n$}{n}次元の単体の体積\texorpdfstring{$\int_{x_1=0}^a \int_{x_2=0}^{a-x_1} \cdots \int_{x_n=0}^{a-x_1- \cdots - x_{n-2}} \mathrm{d}x_n \cdots \mathrm{d}x_2\mathrm{d}x_1 = a^n/n!\;(a>0)$}{}}
			\begin{proof}
				\begin{align*}
