\part{実解析}
	...
	\chapter{微分と積分の関係}
		\section{\texorpdfstring{$\lim_{x\to\infty}{f(x)}$}{lim x→∞ f(x)}が存在すれば\texorpdfstring{$\int_{0}^{\infty}{f'(x) dx} = f(\infty)-f(0)$}{∫\_0\string^∞ f'(x) = f(∞) - f(0)}}
			...
			\end{proof}
		\section{\texorpdfstring{$\derivLong{\integ{a}{x}{f(x)}{x}}{x}{} = f(x)$}{(d/dx)∫\_a\string^x f(x) dx = f(x)}}
			\begin{shadebox}
				連続関数$f(x)$について
		...
		\section{
			...
			\texorpdfstring{$\derivLong{\integ{s=0}{x}{f(s)g(x-s)}{s}}{x}{} = f(x)g(0) + \integ{s=0}{x}{f(s)\left.\deriv{g(x)}{x}{}\right|_{x=x-s}}{s}$}{}
		}
			\begin{proof}
		...
		\section{
			...
			\small{
				\texorpdfstring{$\derivLong{\left(\integ{x_1=0}{x}{\cdots\integ{x_n=0}{x-x_1-\cdots-x_{n-1}}{f_1(x_1)\cdots f_n(x_n)}{x_n}\cdots}{x_1}\right)}{x}{}$}{} \texorpdfstring{\\}{}
				\texorpdfstring{$= \int_{x_1=0}^x\cdots\int_{x_{n-1}=0}^{x-x_1-\cdots -x_{n-2}} f_1(x_1)\cdots f_{n-1}(x_{n-1})f_n(x-x_1-\cdots x_{n-1}) \mathrm{d}x_{n-1}\cdots\mathrm{d}x_{1}$}{}
			}
		}
			\label{重積分の微分}
			\begin{shadebox}
		...
		\section{
			系\texorpdfstring{\\}{}
			\small{
				\texorpdfstring{$\derivLong{\left(\integ{x_1=-\infty}{\infty}{\cdots\integ{x_n=-\infty}{x-x_1-\cdots-x_{n-1}}{f_1(x_1)\cdots f_n(x_n)}{x_n}\cdots}{x_1}\right)}{x}{}$\\}{}
				\texorpdfstring{$= \integ{x_1=-\infty}{\infty}{\cdots\integ{x_{n-1}=-\infty}{x-x_1-\cdots-x_{n-2}}{f_1(x_1)\cdots f_{n-1}(x_{n-1})f_n(x-x_1-\cdots x_{n-1})}{x_{n-1}}\cdots}{x_1}$}{}
			}
		}
			\label{重積分の微分の系}
			\begin{shadebox}
