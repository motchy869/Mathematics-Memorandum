\chapter{凸関数}
    \section{諸定理}
        ...
        \subsection{重み付き相加,相乗平均の関係とYoungの不等式}
            ...
            \end{shadebox}
        \subsection{算術平均と調和平均の関係}
            \begin{shadebox}
                $n\in\naturalNumbers,\;x_1,\dotsc,x_n>0$ とするとき、これらの算術平均は調和平均を下回ることはない。
                つまり次式が成り立つ。
                \[ \frac{1}{n} \sum_{i=1}^n x_i \geq \frac{n}{\sum_{i=1}^n 1/x_i} \]
            \end{shadebox}
            \begin{proof}
                \quad\par
                関数 $f:x\in\setComprehension{x\in\realNumbers}{x>0}\mapsto 1/x$ は狭義下に凸であるから Jensen の不等式より次式が成り立つ。
                \[
                    \sum_{i=1}^n \frac{1}{n} f(x_i) \geq f\left(\sum_{i=1}^n \frac{1}{n} x_i\right) \tag{1}
                \]
                等号成立の必要十分条件は $x_1=x_2=\dots=x_n$ である。
                式(1)を変形すると次式を得る。
                \[ \frac{1}{n} \sum_{i=1}^n 1/x_i \geq \frac{1}{\frac{1}{n} \sum_{i=1}^n x_i} \quad \therefore \frac{1}{n} \sum_{i=1}^n x_i \geq \frac{n}{\sum_{i=1}^n 1/x_i} \]
            \end{proof}
        \subsection{2階導関数と凸性の関係}
            ...
            \begin{proof}
                ...
                $1\Rightarrow 2$を示す。
                $x\in I,x\neq x_1$とする。
                $g(x) \coloneqq f(x) - \bigl(f(x_1) - f^{(1)}(x_1)(x-x_1)\bigr)$とすると$g^{(1)}(x) = f^{(1)}(x) - f^{(1)}(x_1)$なので$g^{(1)}(x_1) = 0$である。
                ...
                \end{align*}
                よって$x_4 \coloneqq (x_1 + x_2)/2$とすると$f(x_4) \geq f(x_1) + f^{(1)}(x_1)(x_4-x_1)$となる。
