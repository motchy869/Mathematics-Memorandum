\chapter{置換}
    \section{置換の符号の定理}
        \subsection{\texorpdfstring{$\sigma'(i) \coloneq \sigma(n+1-i)$としたときの$\sgn\parens*{\sigma'}$}{σ'(i) := σ(n+1-i)としたときのsgn(σ')}}
            \label{置換の引数を逆順にしたときの符号}
            \begin{shadebox}
                $\sigma$を$S=\{1,\dotsc,n\}$上の置換とする。
                $S$上の置換$\sigma'$を$\sigma'(i) = \sigma(n+1-i)$で定めるとき、$\sgn\parens*{\sigma'}$は次式となる。
                \begin{align*}
                    \sgn\parens*{\sigma'} &= s\sgn\parens*{\sigma} \\
                    s &=
                    \begin{cases}
        ...
        \subsection{\texorpdfstring{$\sigma'(i) \coloneq n+1-\sigma(i)$としたときの$\sgn\parens*{\sigma'}$}{σ'(i) := n+1-σ(i)おしたときのsgn(σ')}}
            \label{出力を逆順にしたときの置換の符号}
            \begin{shadebox}
                $\sigma$を$S=\{1,\dotsc,n\}$上の置換とする。
                $S$上の置換$\sigma'$を$\sigma'(i) = n+1-\sigma(i)$で定めるとき、$\sgn\parens*{\sigma'}$は次式となる。
                \begin{align*}
                    \sgn\parens*{\sigma'} &= s\sgn\parens*{\sigma}
                \end{align*}
            ...
            \begin{proof}
                ...
                但し$b_0 \coloneq \epsilon$(単位置換)とする。
                $\sigma_0 \coloneq b_0,\;\sigma_l \coloneq b_l\sigma_{l-1}\;(l=1,2,\dotsc,k)$とする。
                \par
                $\sigma'_0$は逆順にする置換である。
                $1,2,\dotsc,n$を左右の端同士で交換する処理を外側から内側に向かって進める方針で考えれば$\sigma'_0$の符号は定理の主張の$s$に等しいことが解る。
                \par
                $\sigma'_l$と$\sigma'_{l-1}$を比較すると、$b_l$に対応する部分が交換されているので$\sgn\parens*{\sigma'_l} = -\sgn\parens*{\sigma'_{l-1}}$である。
                これより$\sgn\parens*{\sigma'} = \sgn\parens*{\sigma'_k} = (-1)^k\sgn\parens*{\sigma'_0} = \sgn\parens*{\sigma} s$
            \end{proof}
            Mathematicaを用いた数値例による検証は\verb|theorem_sign_of_permutation.nb|を参照