	...
	\chapter{多項式}
		...
		\section{原始多項式}
			\subsection{$\sum_{i=0}^n a_ix^i$が原始多項式ならば$\sum_{i=0}^n a_{n-i}x^i$も原始多項式である}
				\begin{shadebox}
					$p$を素数, $m$を自然数とする。
					有限体$\GaloisField{p^m}$上の多項式$p(x) := \sum_{i=0}^n a_ix^i$が原始多項式であれば、係数の順序を逆にした多項式$p_2(x) := \sum_{i=0}^n a_{n-i}x^i$もまた原始多項式である。
				\end{shadebox}
				\begin{proof}
					\quad\par
					背理法で示す。
					$p_2(x)$が原始多項式でないと仮定すると、ある$l\in\{1,2,\cdots,p^m-2\}$と$\GaloisField{p^m}$上の多項式$q(x) = \sum_{j=0}^{l-n} b_jx^j$が存在して次式が成り立つ。
					\[ x^l = q(x)p_2(x) + 1 \]
