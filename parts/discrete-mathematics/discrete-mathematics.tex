\part{離散数学}
	...
	\chapter{多項式}
		...
		\section{$\sum_{i=0}^n a_ix^i$が既約多項式ならば$\sum_{i=0}^n a_{n-i}x^i$も既約多項式である}
		\label{既約多項式の係数の順序反転}
		\begin{shadebox}
			$p$を素数とする。
			有限体$\GaloisField{p}$上の$n$次多項式$f_1(x) := \sum_{i=0}^n a_ix^i$が既約であれば、係数の順序を逆にした多項式$f_2(x) := \sum_{i=0}^n a_{n-i}x^i$もまた既約である。
		\end{shadebox}
			\begin{proof}
				\quad\par
				背理法で示す。
				$f_2(x)$が既約でないと仮定すると、$\GaloisField{p}$上の適当な多項式$g_{2,1}(x),g_{2,2}(x),\;d_{2,1} + d_{2,2} := \deg(g_{2,1}(x)) + \deg(g_{2,1}(x)) = n$が存在して次式が成り立つ。
				\[ f_2(x) = g_{2,1}(x)g_{2,2}(x) \]
				上式をLaurent多項式に拡張して両辺の$x$を$x^{-1}$で置き換えると
				\[ f_2(x^{-1}) = g_{2,1}(x^{-1})g_{2,2}(x^{-1}) \]
				両辺に$x^n$を掛けると
				\[ f_1(x) = x^n f_2(x) = x^{d_{2,1}}g_{2,1}(x^{-1})x^{d_{2,2}}g_{2,2}(x^{-1}) =: g_{1,1}(x)g_{1,2}(x) \]
				ここに$g_{1,1}(x), g_{1,2}(x)$は$g_{2,1}(x), g_{2,2}(x)$の係数の順序を逆にしたものである。
	...
	\chapter{有限体}
		...
		\section{原始多項式}
			\subsection{$\sum_{i=0}^n a_ix^i$が原始多項式ならば$\sum_{i=0}^n a_{n-i}x^i$も原始多項式である}
				\begin{shadebox}
					$p$を素数とする。
					有限体$\GaloisField{p}$上の$n$次多項式$f_1(x) := \sum_{i=0}^n a_ix^i$が原始多項式であれば、係数の順序を逆にした多項式$f_2(x) := \sum_{i=0}^n a_{n-i}x^i$もまた原始多項式である。
				\end{shadebox}
				\begin{proof}
					\quad\par
					\ref{既約多項式の係数の順序反転}より$f_2(x)$は既約であるから、$f_2(x) \mid x^l - 1$となる最小の自然数$l$が$p^m-1$であることを示せば良い。
					\par
					まず$f_2(x) \mid x^{p^n-1}-1$を示す。
					$f_1(x)$が原始多項式だから$\GaloisField{p}$上の適当な$p^n-1-n$次多項式$g_1(x)$が存在して次式が成り立つ。
					...
					ここに$g_2(x)$は$g_1(x)$の係数の順序を逆にしたものである。
					\par
					次に$l=1,2,\cdots,p^n-2$について$f_2(x) \nmid x^l-1$を背理法で示す。
					仮にある$l$に対して$\GaloisField{p}$上の$l-n$次多項式$h_1(x)$が存在して
					\[ x^l - 1 = h_1(x)f_2(x) \]
					が成り立つとする。
					上式をLaurent多項式に拡張して両辺の$x$を$x^{-1}$で置き換えると
					...
					ここに$h_2(x)$は$h_1(x)$の係数の順序を逆にしたものである。
					これは$f_1(x)$が原始多項式であることに矛盾する。
					\par
					以上より$f_2(x) \mid x^l - 1$となる最小の自然数$l$は$p^m-1$である。
				\end{proof}
			\subsection{形式的冪級数に於ける原始多項式の逆数の係数列は周期的}
				\begin{shadebox}
					$p$を素数とする。
					有限体$\GaloisField{p}$上の形式的冪級数に於いて、$n$次の原始多項式$f(x)$の逆数$f(x)^{-1}$の係数列は周期的であり、その周期は$p^n-1$である。
				\end{shadebox}
				\begin{proof}
					\quad\par
					まず係数列が周期的であることを示す。
					$f(x)$が原始多項式だから、$\GaloisField{p}$上の適当な$p^n-1-n$次多項式$g(x)$が存在して次式が成り立つ。
					\begin{align*}
						x^{p^n-1} - 1 &= g(x)f(x) \\
					...
					\par
					仮に周期が$p^n-1$未満であるとする、
					つまりある$l\in\{1,2,\cdots,p^n-2\}$と$\GaloisField{p}$上の適当な$l$次未満の多項式$h(x)$が存在して
