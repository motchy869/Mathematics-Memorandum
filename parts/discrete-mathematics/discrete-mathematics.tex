		...
		\section{原始多項式}
			\subsection{$\sum_{i=0}^n a_ix^i$が原始多項式ならば$\sum_{i=0}^n a_{n-i}x^i$も原始多項式である}
				...
				\begin{proof}
					\quad\par
					背理法で示す。
					$f_2(x)$が原始多項式でないと仮定すると、ある$l\in\{1,2,\cdots,p^n-2\}$と$\GaloisField{p}$上の多項式$g(x) = \sum_{j=0}^{l-n} b_jx^j$が存在して次式が成り立つ。
					\[ x^l = g(x)f_2(x) + 1 \]
					ここで上式をLaurent多項式に拡張して両辺の$x$を$1/x$で置き換えると
					\[ \frac{1}{x^l} = \left(\sum_{j=0}^{l-n} b_jx^{-j}\right)\left(\sum_{i=0}^n a_{n-i}x^{-i}\right) + 1 \]
					両辺に$x^l$を掛けると
					\[ 1 = \left(\sum_{j=0}^{l-n} b_jx^{l-n-j}\right)\left(\sum_{i=0}^n a_{n-i}x^{n-i}\right) + x^l = \left(\sum_{j=0}^{l-n} b_jx^{l-n-j}\right)f_1(x) + x^l \]
					\[ \therefore x^l \equiv 1 \mod f_1(x) \]
