\part{離散数学}
	...
	\chapter{公式}
		...
		\section{M\"obiusの反転公式}
			\begin{shadebox}
				...
				任意の自然数$n$に対して$g(n) = \sum_{d \mid n}f(d)$が成り立つならば$f(n) = \sum_{d \mid n} \mu(d)g(n/d) = \sum_{d \mid n} \mu(n/d)g(n)$が成り立つ。
			\end{shadebox}
			\begin{proof}
				\quad\par
				...
				1つ目の等号成立を示す。
				\begin{align*}
					\sum_{d \mid n} \mu(d)g(n/d) &= \sum_{d \mid n} \mu(d)\sum_{e \mid n/d} f(e) = \sum_{d \mid n} \sum_{e \mid n/d} \mu(d)f(e) = \sum_{d,e \atop de \mid n} \mu(d)f(e) \\
					&= \sum_{e \mid n} \sum_{d \mid n/e} \mu(d)f(e) = \sum_{e \mid n} f(e) \sum_{d \mid n/e} \mu(d) = \sum_{e \mid n} f(e) \indII{e=n} = f(n)
				\end{align*}
			\end{proof}
	\chapter{数え上げ}
		\section{``for $i=1, \cdots, n(\in \naturalNumbers)$''は``for $d \mid n, \text{ for } i=1,\cdots,n \text{ where } \gcd(i,n)=d$''と同じ}
			1から$n$までの自然数に対して、それと$n$との最大公約数は$n$の約数のどれかと一致するから、主張が成り立つ。
			次のような使い方が考えられる。
			\begin{itemize}
				\item $\sum_{i=1}^n f(i) = \sum_{d \mid n} \sum_{i=1, \cdots, n \atop \gcd(i,n)=d} f(i)$
				\item $\prod_{i=1}^n f(i) = \prod_{d \mid n} \prod_{i=1, \cdots, n \atop \gcd(i,n)=d} f(i)$
			\end{itemize}
		\section{包除原理}
	...
	\chapter{有限体}
		\section{素数位数の有限体の構成法}
			...
			あとは、$*'$に関する唯一の逆元が存在すること、すなわち任意の$a\in S$に対して唯一の$b\in S$が存在して$a*'b=1$となることを示せばよいのだが、\ref{a,mが互いに素のとき、0,a,...a(m-1)をmで割った余りは全て異なる}より、それが保証される。
			\par
		...
		\section{位数が素数の冪乗である有限体の構成法}
			\subsection{既約多項式を用いる方法}
				...
				あとは、$*'$に関する唯一の逆元が存在すること、すなわち任意の$a\in S$に対して唯一の$b\in S$が存在して$a*'b=1$となることを示せばよい。
				少なくとも一つ存在することはBezoutの等式により保証される。
				唯一であることを背理法で示す。
			...
			\subsection{既約多項式の根を用いる方法(根体)}
				...
				あとは、乗法に関する唯一の逆元が存在すること、すなわち任意の$a\in S$に対して唯一の$b\in S$が存在して$ab=1$となることを示せばよい。
				少なくとも一つ存在することは、$a$に現れる$\alpha$を不定元$x$で置換して多項式に戻してからBezoutの等式を考えることで理解できる。
				唯一であることは背理法で示せる。
